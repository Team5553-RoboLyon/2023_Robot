...
#include "lib/N/Geometry/Components/Specifications/NGeometryMesh_Flags.h"
...
extern "C"
{
	...
	inline void NCopyMesh(NMESH *pmeshdst, const NMESH *pmeshsrc)
	{
		NCopyArray(&pmeshdst->VertexArray, &pmeshsrc->VertexArray);
		NCopyArray(&pmeshdst->PrimitiveArray, &pmeshsrc->PrimitiveArray);
	}
	inline void NEraseMesh(NMESH *pmesh)
	{
		NEraseArray(&pmesh->PrimitiveArray, NULL);
		NEraseArray(&pmesh->VertexArray, NULL);
	}
	...
	inline NLINE *NGetFirstMeshLinePtr(const NMESH *pmesh)
	{
		NErrorIf(pmesh->PrimitiveArray.ElementSize != sizeof(NLINE), NERROR_GEOMETRY_WRONG_PRIMITIVE_ARRAY_SIZE);
		return (NLINE *)pmesh->PrimitiveArray.pFirst;
	}
	inline NTRIANGLE *NGetFirstMeshTrianglePtr(const NMESH *pmesh)
	{
		NErrorIf(pmesh->PrimitiveArray.ElementSize != sizeof(NTRIANGLE), NERROR_GEOMETRY_WRONG_PRIMITIVE_ARRAY_SIZE);
		return (NTRIANGLE *)pmesh->PrimitiveArray.pFirst;
	}
...
#define NPushBackMeshPrimitive(pmesh, pprimitive, primitivestructsize) NPushBackMeshPrimitive_debug(pmesh, pprimitive, primitivestructsize)
	...
	inline NBYTE *NPushBackMeshVertex_debug(NMESH *pmesh, const NBYTE *pvertex, const Nu32 vertexstructsize)
	{
		NErrorIf(NGetArrayElementSize(&pmesh->VertexArray) != vertexstructsize, NERROR_GEOMETRY_WRONG_VERTEX_STRUCTURESIZE);
		return (NBYTE *)NArrayPushBack(&pmesh->VertexArray, (NBYTE *)pvertex);
	}
	inline NBYTE *NPushBackMeshPrimitive_debug(NMESH *pmesh, const NBYTE *pprimitive, const Nu32 primitivestructsize)
	{
		NErrorIf(pmesh->PrimitiveArray.ElementSize != primitivestructsize, NERROR_GEOMETRY_WRONG_PRIMITIVE_STRUCTURESIZE);
		return (NBYTE *)NArrayPushBack(&pmesh->PrimitiveArray, (NBYTE *)pprimitive);
	}
#endif
#ifndef _DEBUG
