...
#include "lib/N/Geometry/Components/NGeometrytextureUnit.h"
#include "lib/N/Geometry/Components/NGeometryMaterial.h"
#include "lib/N/Geometry/Components/NGeometryBlend.h"
#include "lib/N/Geometry/Components/Specifications/NGeometryAppearance_Flags.h"
...
#define NAPPEARANCE_CHECKMASK_DIFFERENT 2 // Appearance Raw member and corresponding checked Appearance member are going to be compare together and must be DIFFERENT.
	...
	// ***************************************************************************************
	inline void NSetAppearance_Raw_Texture(NAPPEARANCE_RAW *pdesc, const Nu8 tu, const NTEXTURE *ptexture)
	{
		NErrorIf(tu >= CONSTANT_NAPPEARANCE_RAW_TEXTUREUNIT_NUMBER, NERROR_GEOMETRY_UNSUPPORTED_TEXTUREUNIT_NUMBER);
		pdesc->TextureUnit[tu].pTexture = (NTEXTURE *)ptexture;
	}
	inline void NSetAppearance_Raw_TextureMatrixAnim(NAPPEARANCE_RAW *pdesc, const Nu8 tu, const NUVMATRIXANIM *pmu)
	{
		NErrorIf(tu >= CONSTANT_NAPPEARANCE_RAW_TEXTUREUNIT_NUMBER, NERROR_GEOMETRY_UNSUPPORTED_TEXTUREUNIT_NUMBER);
		pdesc->TextureUnit[tu].pUVMatrixAnim = (NUVMATRIXANIM *)pmu;
	}
	inline void NSetAppearance_Raw_BlendAndMaterial(NAPPEARANCE_RAW *pdesc, const NBLEND *pblend, const NMATERIAL *pmaterial)
	{
		pdesc->pBlend = (NBLEND *)pblend;
		pdesc->pMaterial = (NMATERIAL *)pmaterial;
	}
	...
	inline NTEXTURE *NGetAppearance_Raw_Texture(NAPPEARANCE_RAW *pdesc, const Nu8 tu)
	{
		NErrorIf(tu >= CONSTANT_NAPPEARANCE_RAW_TEXTUREUNIT_NUMBER, NERROR_GEOMETRY_UNSUPPORTED_TEXTUREUNIT_NUMBER);
		return pdesc->TextureUnit[tu].pTexture;
	}
	inline NUVMATRIXANIM *NGetAppearance_Raw_TextureMatrixAnim(NAPPEARANCE_RAW *pdesc, const Nu8 tu)
	{
		NErrorIf(tu >= CONSTANT_NAPPEARANCE_RAW_TEXTUREUNIT_NUMBER, NERROR_GEOMETRY_UNSUPPORTED_TEXTUREUNIT_NUMBER);
		return pdesc->TextureUnit[tu].pUVMatrixAnim;
	}
