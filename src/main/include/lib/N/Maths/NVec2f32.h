...
extern "C"
{
	...
	inline void NVec2Add(NVEC2 *vr, const NVEC2 *v1, const NVEC2 *v2)
	{
		vr->x = v1->x + v2->x;
		vr->y = v1->y + v2->y;
	}
	inline void NVec2AddTo(NVEC2 *v1, const NVEC2 *v2)
	{
		v1->x += v2->x;
		v1->y += v2->y;
	}
	...
	inline void NVec2SubFrom(NVEC2 *v1, const NVEC2 *v2)
	{
		v1->x -= v2->x;
		v1->y -= v2->y;
	}
	inline void NVec2Null(NVEC2 *v)
	{
		v->x = 0;
		v->y = 0;
	}
	inline Nbool NVec2IsUnit(const NVEC2 *v) { return (NABS(1.0f - NVec2Length(v)) < NF32_EPSILON_VECTOR_LENGTH ? 1 : 0); }
	inline void NVec2Lerp(NVEC2 *vr, const NVEC2 *vorigin, const NVEC2 *v, const Nf32 t)
	{
