...
inline void NSetThnAnimKey_from_Thn(NTHNANIM_KEY *pthnkey,const NTRANSFORM_HNODE* pthn, const Nf32 time, const NEASING_HANDLE_ID_ENUM easing){pthnkey->Time = time;pthnkey->EasingHandleID = easing;pthnkey->Translation = pthn->vTranslation;pthnkey->Scale = pthn->vScale; pthnkey->Rotation = pthn->QuatRotation;}
inline void NSetThnAnimKey_Identity(NTHNANIM_KEY *pthnkey,const Nf32 time, const NEASING_HANDLE_ID_ENUM easing){pthnkey->Time = time;pthnkey->EasingHandleID = easing;NVec3Set(&pthnkey->Translation,0,0,0);NVec3Set(&pthnkey->Scale,1.0f,1.0f,1.0f);NIdentityQuaternion(&pthnkey->Rotation);}
