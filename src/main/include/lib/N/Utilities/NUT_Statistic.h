...
extern "C"
{
	...
	inline Nf32 NUT_GetStatistic_Nf32_Average(const NUT_STATISTIC_Nf32 *pfstat)
	{
		if (pfstat->Samples)
		{
			return pfstat->Sum / pfstat->Samples;
		}
		else
		{
			return 0.0f;
	...
	inline Nu32 NUT_GetStatistic_Nu32_Average(const NUT_STATISTIC_Nu32 *pstat)
	{
		if (pstat->Samples)
		{
			return pstat->Sum / pstat->Samples;
		}
		else
		{
			return 0;
