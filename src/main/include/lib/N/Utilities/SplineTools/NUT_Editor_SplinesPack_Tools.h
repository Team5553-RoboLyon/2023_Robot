...
#define NUT_SPACK_SPLINEKNOT_TB_ENDHANDLE 5
	...
	inline NUT_SPACK_CONSTRAINTS_INTERNVALUES_CALLBACK NUT_SetSplinesPackConstraints_InternalValuesCallBack(NUT_SPACK_TOOLDATA *ps, NUT_SPACK_CONSTRAINTS_INTERNVALUES_CALLBACK internalvalue_callback)
	{
		NUT_SPACK_CONSTRAINTS_INTERNVALUES_CALLBACK old = ps->InternalValues_CallBack;
		ps->InternalValues_CallBack = internalvalue_callback;
		return old;
	}
	...
	inline void NUT_SetSplinesPackToolData_MoveCallBacks(NUT_SPACK_TOOLDATA *ps, const NUT_SPACK_PRE_MOVE_CALLBACK premove, const NUT_SPACK_POST_MOVE_CALLBACK postmove)
	{
		ps->PreMove_CallBack = premove;
		ps->PostMove_CallBack = postmove;
	}
	inline void NUT_SetSplinesPackToolData_AddCallBacks(NUT_SPACK_TOOLDATA *ps, const NUT_SPACK_PRE_ADD_CALLBACK preadd, const NUT_SPACK_POST_ADD_CALLBACK postadd)
	{
		ps->PreAdd_CallBack = preadd;
		ps->PostAdd_CallBack = postadd;
	}
	inline void NUT_SetSplinesPackToolData_SuprCallBacks(NUT_SPACK_TOOLDATA *ps, const NUT_SPACK_PRE_SUPR_CALLBACK presupr, const NUT_SPACK_POST_SUPR_CALLBACK postsupr)
	{
		ps->PreSupr_CallBack = presupr;
		ps->PostSupr_CallBack = postsupr;
	}
	inline void NUT_SetSplinesPackToolData_CreateSplineCallBacks(NUT_SPACK_TOOLDATA *ps, const NUT_SPACK_PRE_CREATE_CALLBACK precrea, const NUT_SPACK_POST_CREATE_CALLBACK postcrea, const NUT_SPACK_CREATE_CALLBACK crea)
	{
		ps->PreCreate_CallBack = precrea;
		ps->PostCreate_CallBack = postcrea;
		ps->Create_CallBack = crea;
	}
	inline void NUT_SetSplinesPackToolData_DeleteSplineCallBacks(NUT_SPACK_TOOLDATA *ps, const NUT_SPACK_PRE_DELETE_CALLBACK predel, const NUT_SPACK_POST_DELETE_CALLBACK postdel, const NUT_SPACK_DELETE_CALLBACK del)
	{
		ps->PreDelete_CallBack = predel;
		ps->PostDelete_CallBack = postdel;
		ps->Delete_CallBack = del;
	}
	...
	inline Nu32 NUT_SplinesPackToolData_BindSpline(NUT_SPACK_TOOLDATA *ptd, const NSPLINE *ps)
	{
		NArrayPushBack(&ptd->ArrayOfSplinesPtr, (NBYTE *)&ps);
		return ptd->ArrayOfSplinesPtr.Size - 1;
	}
	...
	inline void NUT_EraseAllSplinesPackConstraints(NUT_SPACK_TOOLDATA *pt)
	{
		NEraseArray(&pt->Constraints_OpA_Equal_OpB, NULL);
		NEraseArray(&pt->Constraints_OpB_Equal_OpA, NULL);
	}
	...
	inline Nbool NUT_SetSplinesPackValuef(NUT_SPACK_TOOLDATA *pt, const NUT_SPACK_SPLINEPOINT_ENUM internal_value_id, const Nf32 x, const Nf32 y, const Nf32 z)
	{
		if (internal_value_id < NUT_SPACK_INTERNAL_VALUE_NB)
		{
			NVec3Set(&pt->InternalSplinePoint[internal_value_id].Position, x, y, z);
			return NTRUE;
		}
		else
			return NFALSE;
	}
	inline Nbool NUT_SetSplinesPackValueXf(NUT_SPACK_TOOLDATA *pt, const NUT_SPACK_SPLINEPOINT_ENUM internal_value_id, const Nf32 val)
	{
		if (internal_value_id < NUT_SPACK_INTERNAL_VALUE_NB)
		{
			pt->InternalSplinePoint[internal_value_id].Position.x = val;
			return NTRUE;
		}
		else
			return NFALSE;
	}
	inline Nbool NUT_SetSplinesPackValueYf(NUT_SPACK_TOOLDATA *pt, const NUT_SPACK_SPLINEPOINT_ENUM internal_value_id, const Nf32 val)
	{
		if (internal_value_id < NUT_SPACK_INTERNAL_VALUE_NB)
		{
			pt->InternalSplinePoint[internal_value_id].Position.y = val;
			return NTRUE;
		}
		else
			return NFALSE;
	}
	inline Nbool NUT_SetSplinesPackValueZf(NUT_SPACK_TOOLDATA *pt, const NUT_SPACK_SPLINEPOINT_ENUM internal_value_id, const Nf32 val)
	{
		if (internal_value_id < NUT_SPACK_INTERNAL_VALUE_NB)
		{
			pt->InternalSplinePoint[internal_value_id].Position.z = val;
			return NTRUE;
		}
		else
			return NFALSE;
