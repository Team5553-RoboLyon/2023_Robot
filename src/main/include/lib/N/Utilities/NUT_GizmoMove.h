...
inline	void			NUT_SetGizmoMove_SnapOriginf(NUT_GIZMO_MOVE *pgm, const Nf32 ox,const Nf32 oy,const Nf32 oz ){pgm->SnapOrigin.x = ox;pgm->SnapOrigin.y = oy;pgm->SnapOrigin.z = oz;}
inline	void			NUT_SetGizmoMove_SnapStep(NUT_GIZMO_MOVE *pgm, const NVEC3 *pstep){NErrorIf(!pstep->x || !pstep->y || !pstep->z, NERROR_INCONSISTENT_PARAMETERS);pgm->SnapStep = *pstep;}
inline	void			NUT_SetGizmoMove_SnapStepf(NUT_GIZMO_MOVE *pgm,const Nf32 stepx,const Nf32 stepy,const Nf32 stepz ){NErrorIf(!stepx || !stepy || !stepz, NERROR_INCONSISTENT_PARAMETERS);pgm->SnapStep.x = stepx;pgm->SnapStep.y = stepy;pgm->SnapStep.z = stepz;}
inline	void			NUT_SetGizmoMove_SnapStrength(NUT_GIZMO_MOVE *pgm, const Nf32 strength){pgm->SnapStrength = strength;}

inline NVEC3*		NUT_GetGizmoMove_Position( NVEC3 *prm, const NUT_GIZMO_MOVE *pgm ){*prm = *(NVEC3*)&pgm->Matrix.Translation;return prm;}
inline NVEC3*		NUT_GetGizmoMove_RelativeMove( NVEC3 *prm, const NUT_GIZMO_MOVE *pgm ){NVec3Sub(prm,(NVEC3*)&pgm->Matrix.Translation,(NVEC3*)&pgm->PreviousTranslation);return prm;}

inline NVEC3*		NUT_GetGizmoMove_HandlePosition( NVEC3 *prm, const NUT_GIZMO_MOVE *pgm ){*prm = pgm->CurrentHandle;return prm;}
inline NVEC3*		NUT_GetGizmoMove_PreviousHandlePosition( NVEC3 *prm, const NUT_GIZMO_MOVE *pgm ){*prm = pgm->PreviousHandle;return prm;}
