...
#define NCOLOR_PRESET3F_ORANGE 1.000f, 0.255f, 0.000f
...
#define NCOLOR_PRESET3F_GREEN_SPRING 0.000f, 1.000f, 0.500f
...
#define NCOLOR_PRESET3F_ORANGE_GOLD 1.000f, 0.749f, 0.000f
#define NCOLOR_PRESET3F_ORANGE_AMBER 1.000f, 0.843f, 0.000f
...
#define NRANGE_4COLORS 4 // for quad building
	...
	inline NCOLOR *NScaleColor(NCOLOR *pc, const Nf32 scalecomp, const Nf32 scalealpha)
	{
		pc->red *= scalecomp;
		pc->green *= scalecomp;
		pc->blue *= scalecomp;
	...
	inline NCOLOR *NSetColorf(NCOLOR *pc, const Nf32 r, const Nf32 g, const Nf32 b, const Nf32 a)
	{
		pc->red = r;
		pc->green = g;
		pc->blue = b;
	...
	inline NCOLOR *NSetColorRangef(NCOLOR *pfirst, const Nu32 size, const Nf32 r, const Nf32 v, const Nf32 b, const Nf32 a)
	{
		Nu32 i;
		for (i = 0; i < size; i++)
		{
			NSetColorf(&pfirst[i], r, v, b, a);
		}
		return pfirst;
	}
	inline NCOLOR *NSetColorRange(NCOLOR *pfirst, const Nu32 size, const NCOLOR *pcolor)
	{
		Nu32 i;
		for (i = 0; i < size; i++)
		{
	...
	inline NCOLOR *NColorLerp(NCOLOR *cr, const NCOLOR *corigin, const NCOLOR *c, const Nf32 t)
	{
		cr->red = corigin->red + (c->red - corigin->red) * t;
		cr->green = corigin->green + (c->green - corigin->green) * t;
		cr->blue = corigin->blue + (c->blue - corigin->blue) * t;
		cr->alpha = corigin->alpha + (c->alpha - corigin->alpha) * t;
		return cr;
	}
	...
	inline Nu32 *NCOLOR_To_RGBA8(Nu32 *rgba, const NCOLOR *pcolor)
	{
		*rgba = RGBA8((Nu8)(pcolor->red * 255.0f), (Nu8)(pcolor->green * 255.0f), (Nu8)(pcolor->blue * 255.0f), (Nu8)(pcolor->alpha * 255.0f));
		return rgba;
	}
	inline NCOLOR *RGBA8_To_NCOLOR(NCOLOR *pcolor, const Nu32 rgba)
	{
		pcolor->red = (Nf32)RGBA8_R(rgba) / 255.0f;
		pcolor->green = (Nf32)RGBA8_G(rgba) / 255.0f;
		pcolor->blue = (Nf32)RGBA8_B(rgba) / 255.0f;
		pcolor->alpha = (Nf32)RGBA8_A(rgba) / 255.0f;
		return pcolor;
	}

	inline Nu32 *NCOLOR_To_RGB8(Nu32 *rgb8, const NCOLOR *pcolor)
	{
		*rgb8 = RGB8((Nu8)(pcolor->red * 255.0f), (Nu8)(pcolor->green * 255.0f), (Nu8)(pcolor->blue * 255.0f));
		return rgb8;
	}
	inline NCOLOR *RGB8_To_NCOLOR(NCOLOR *pcolor, const Nu32 rgb, const Nf32 alpha)
	{
		pcolor->red = (Nf32)RGB8_R(rgb) / 255.0f;
		pcolor->green = (Nf32)RGB8_G(rgb) / 255.0f;
		pcolor->blue = (Nf32)RGB8_B(rgb) / 255.0f;
		if (alpha >= 0.0f)
		{
			pcolor->alpha = alpha;
		}
	...
	inline Nu32 *NCOLOR_To_RGB5(Nu32 *rgb5, const NCOLOR *pcolor)
	{
		*rgb5 = RGB5((Nu8)(pcolor->red * 31.0f), (Nu8)(pcolor->green * 31.0f), (Nu8)(pcolor->blue * 31.0f));
		return rgb5;
	}
	inline NCOLOR *RGB5_To_NCOLOR(NCOLOR *pcolor, const Nu32 rgb5, const Nf32 alpha)
	{
		pcolor->red = (Nf32)RGB5_R(rgb5) / 31.0f;
		pcolor->green = (Nf32)RGB5_G(rgb5) / 31.0f;
		pcolor->blue = (Nf32)RGB5_B(rgb5) / 31.0f;
		if (alpha >= 0.0f)
		{
			pcolor->alpha = alpha;
		}
	...
	inline Nu32 *NCOLOR_To_GRAYA8(Nu32 *graya, const NCOLOR *pcolor)
	{
		*graya = GRAYA8((Nu8)((pcolor->red + pcolor->green + pcolor->blue) * (255.0f / 3.0f)), (Nu8)(pcolor->alpha * 255.0f));
		return graya;
	}
	inline NCOLOR *GRAYA8_To_NCOLOR(NCOLOR *pcolor, const Nu32 graya8)
	{
		pcolor->red = pcolor->green = pcolor->blue = (Nf32)GRAYA8_G(graya8) / 255.0f, (Nf32)GRAYA8_A(graya8) / 255.0f;
		return pcolor;
	}
