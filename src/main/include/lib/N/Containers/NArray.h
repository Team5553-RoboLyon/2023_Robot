...
#define NARRAY_INCREASE_CAPACITY_RATIO 1.0f // new capacity = oldcapacity + oldcapacity * NARRAY_INCREASE_CAPACITY_RATIO + 1
	...
	inline NARRAYBOUNDS *NGetArrayBounds(NARRAYBOUNDS *pab, NARRAY *parr)
	{
		pab->Capacity = parr->Capacity;
		pab->ElementSize = parr->ElementSize;
		pab->Size = parr->Size;
		return pab;
	}
...
#define NIsArrayAwkward(parr, pb, b) (NCheckArrayIntegrity(parr, pb, b) < 0 ? 1 : 0)
	...
	{
		if (ptr < parray->pFirst)
			return NFALSE;
		if (!parray->Size)
			return NFALSE;
		if (!parray->ElementSize)
			return NFALSE;
		Nu32 id = (ptr - parray->pFirst) / parray->ElementSize;
		if (id > (parray->Size - 1))
			return NFALSE;
		if (ptr != (parray->pFirst + id * parray->ElementSize))
			return NFALSE;
		return NTRUE;
	}

	inline Nbool NIsValidArrayElementIndex(const NARRAY *parray, const Nu32 id)
	{
		if (id < parray->Size)
			return NTRUE;
		else
	...
	inline NBYTE *NGetLastArrayPtr(const NARRAY *parray)
	{
		if (parray->Size)
			return parray->pFirst + (parray->Size - 1) * parray->ElementSize;
		else
			return NULL;
	}
	inline NBYTE *NGetArrayPtr(const NARRAY *parray, const Nu32 index)
	{
		NErrorIf(index >= parray->Size, NERROR_ARRAY_INDEX_BEYOND_LIMITS);
		return parray->pFirst + index * parray->ElementSize;
	}
	...
	inline Nu32 NGetArrayIndex(const NARRAY *parray, const NBYTE *ptr)
	{
		NErrorIf(!NIsValidArrayElementPtr(parray, ptr), NERROR_ARRAY_INDEX_BEYOND_LIMITS);
		return ((ptr - parray->pFirst) / parray->ElementSize);
	}
...
#define NEnsureArrayCapacity(parray, capa)                           \
	if ((parray)->Capacity < (capa))                                 \
	{                                                                \
		NIncreaseArrayCapacity(parray, (capa) - (parray)->Capacity); \
	}
	...
	inline Nbool NIsArrayCleared(const NARRAY *parray)
	{
		if (NGetArrayCapacity(parray) || NGetArraySize(parray) || NGetFirstArrayPtr(parray) || NGetArrayElementSize(parray))
		{
			return NFALSE;
		}
		else
