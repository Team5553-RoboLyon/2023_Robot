...
inline	void*	NGetRingBufferHeadRelativeIndexPtr(const NRINGBUFFER *pringbuffer, const Nu32 id){NErrorIf(!NIsValidRingBufferRelativeIndex(pringbuffer,id), NERROR_RINGBUFFER_INDEX_OUTOFRANGE);return (NBYTE*)pringbuffer->pBuffer + ((pringbuffer->Head + id)%pringbuffer->Capacity)*pringbuffer->ElementSize;}
