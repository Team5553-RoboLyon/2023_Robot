...
#define NNEWTON_METHOD_MAX_ITER 32
	...
	inline Nu32 NPowerOfTwoCeil(Nu32 x)
	{
		if (x <= 1)
			return 1;
		Nu32 pw = 2;
	...
	inline Nu32 NPowerOfTwoFloor(Nu32 x)
	{
		Nu32 pw = 2;
		while (x >>= 1)
			pw <<= 1;
	...
	inline Nf32 NModulo2PIf32(Nf32 angle)
	{
		angle = fmod(angle, NF32_2PI);
		if (angle < 0.0f)
		{
			angle += NF32_2PI;
		}
	...
	inline Nf64 NModulo2PIf64(Nf64 angle)
	{
		angle = fmod(angle, NF64_2PI);
		if (angle < 0.0)
		{
			angle += NF64_2PI;
		}
