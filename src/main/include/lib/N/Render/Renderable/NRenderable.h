...
inline void						NSetRenderableTransformHNode( NRENDERABLE *pr, NTRANSFORM_HNODE *pthn ){if(pr->pTransformHNode){NUnregisterTransformHNodeReference(pr->pTransformHNode);} if(pthn){NRegisterTransformHNodeReference(pthn);} pr->pTransformHNode = pthn;}
...
inline	NGEOMETRY*				NGetFirstIncludedGeometry(const NRENDERABLE *pr){NErrorIf(!pr->GeomArray.Size,NERROR_ARRAY_ELEMENT_NOT_FOUND);return (NGEOMETRY*)pr->GeomArray.pFirst;/* *(NGEOMETRY*)NGetFirstArrayPtr(&pr->GeomArray)*/;}
inline	NGEOMETRY*				NGetIncludedGeometryByIndex(const NRENDERABLE *pr,const Nu32 index){NErrorIf(index>=pr->GeomArray.Size,NERROR_ARRAY_INDEX_BEYOND_LIMITS);return (NGEOMETRY*)NGetArrayPtr(&pr->GeomArray,index);}
Nu32							NGetIncludedGeometryIndex(const NRENDERABLE *pr,const NGEOMETRY *pgeom);

// replace NRenderableGeometryPushBack
inline	NGEOMETRY*				NGeometryEmplaceBack(NRENDERABLE *pr, NGEOMETRY_DESC* pgdesc){NUpSizeArray(&pr->GeomArray,1,NULL);return NSetupGeometry( (NGEOMETRY*)NGetLastArrayPtr(&pr->GeomArray),pgdesc);}
