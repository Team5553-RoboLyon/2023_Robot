...
#define FLAG_NTOUCH_LISTENER_TO_BE_CLEAN BIT_0
	...
	// + Touch
	inline NLISTENED_TOUCH *NGetListenedTouch(NTOUCH_LISTENER *plistener, const Nu32 touch_id)
	{
		NLISTENED_TOUCH *plt = plistener->pLTouchBuffer;
		for (Nu8 j = plistener->LTouchBufferSize; j != 0; j--, plt++)
		{
			if (plt->SystemID == touch_id)
			{
				return plt;
			}
		}
