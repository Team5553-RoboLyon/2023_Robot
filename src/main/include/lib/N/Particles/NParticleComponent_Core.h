...
#define FLAGS_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_ALL 0x1FF
	...
	inline void NSetupParticleEmitterComponent_Core_Desc_HighLevelExtractSet(NPARTICLE_EMITTERCOMPONENT_CORE_DESC *pdesc, const NEXTRACTSET hxset)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_HIGHLEVEL_EXTRACTSET);
		pdesc->HighLevelExtractSet = (NEXTRACTSET)hxset;
	}
	...
	inline void NSetupParticleEmitterComponent_Core_Desc_BoundingSphere(NPARTICLE_EMITTERCOMPONENT_CORE_DESC *pdesc, const NBOUNDINGSPHERE *psh)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_BOUNDINGSPHERE);
		pdesc->BoundingSphere = *(NBOUNDINGSPHERE *)psh;
	}
	inline void NSetupParticleEmitterComponent_Core_Desc_BoundingSpheref(NPARTICLE_EMITTERCOMPONENT_CORE_DESC *pdesc, const Nf32 radius, const Nf32 centerx, const Nf32 centery, const Nf32 centerz)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_BOUNDINGSPHERE);
		NVec3Set(&pdesc->BoundingSphere.Center, centerx, centery, centerz);
		pdesc->BoundingSphere.fRadius = radius;
	}
	inline void NSetupParticleEmitterComponent_Core_Desc_ParticleNumbers(NPARTICLE_EMITTERCOMPONENT_CORE_DESC *pdesc, const Nu16 particlesnbmax, const Nu16 initialparticlesnb)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_PARTICLESNBMAX);
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_INITIALPARTICLESNB);
		pdesc->ParticlesNbMax = particlesnbmax;
		pdesc->InitialParticlesNb = initialparticlesnb;
	}
	inline void NSetupParticleEmitterComponent_Core_Desc_BirthRate(NPARTICLE_EMITTERCOMPONENT_CORE_DESC *pdesc, const Nf32 birthrate)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_BIRTH_RATE);
		pdesc->BirthRate = birthrate;
	}
	inline void NSetupParticleEmitterComponent_Core_Desc_TransformHNode(NPARTICLE_EMITTERCOMPONENT_CORE_DESC *pdesc, const NTRANSFORM_HNODE *pthn)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_THN);
		pdesc->pTransformHNode = (NTRANSFORM_HNODE *)pthn;
	}

	inline void NSetupParticleEmitterComponent_Core_Desc_Texture(NPARTICLE_EMITTERCOMPONENT_CORE_DESC *pdesc, const Nu8 tuid, const NTEXTURE *ptexture)
	{
		NErrorIf(!ptexture, NERROR_NULL_POINTER);
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_APPERANCERAW);
		NSetAppearance_Raw_Texture(&pdesc->AppearanceRaw, tuid, ptexture);
	}
	inline void NSetupParticleEmitterComponent_Core_Desc_BlendAndMaterial(NPARTICLE_EMITTERCOMPONENT_CORE_DESC *pdesc, const NBLEND *pblend, const NMATERIAL *pmaterial)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_CORE_DESC_USE_APPERANCERAW);
