...
#define FLAG_NPARTICLE_EMITTERCOMPONENT_LUT_DESC_USE_ALL 0x3
	...
	// **								 Functions											**
	// ***************************************************************************************
	inline void NSetupParticleEmitterComponent_LUT_Desc_LUTSize(NPARTICLE_EMITTERCOMPONENT_LUT_DESC *pdesc, const Nu16 lutsize)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_LUT_DESC_USE_LUT_SIZE);
		pdesc->LUT_Size = lutsize;
	}
	inline void NSetupParticleEmitterComponent_LUT_Desc_LUTInitial(NPARTICLE_EMITTERCOMPONENT_LUT_DESC *pdesc, const Nf32 scale, const NCOLOR *pc)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_LUT_DESC_USE_LUT_INITIAL);
		pdesc->LUT_Initial.Scale = scale;
		pdesc->LUT_Initial.Color = *pc;
	}
	inline void NSetupParticleEmitterComponent_LUT_Desc_LUTInitialf(NPARTICLE_EMITTERCOMPONENT_LUT_DESC *pdesc, const Nf32 scale, const Nf32 red, const Nf32 green, const Nf32 blue, const Nf32 alpha)
	{
		FLAG_ON(pdesc->Flags, FLAG_NPARTICLE_EMITTERCOMPONENT_LUT_DESC_USE_LUT_INITIAL);
		pdesc->LUT_Initial.Scale = scale;
		pdesc->LUT_Initial.Color.red = red;
		pdesc->LUT_Initial.Color.green = green;
		pdesc->LUT_Initial.Color.blue = blue;
		pdesc->LUT_Initial.Color.alpha = alpha;
	}
