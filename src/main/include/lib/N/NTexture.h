...
#define MASK_NTEXTURE_ALPHA_CHANNEL_INFOS 0x7F // BIT_0 to BIT_6 included
	...
	inline Nbool NIsTextureAlphaEffective(const NTEXTURE *pt)
	{
		if (ISFLAG_ON(pt->Flags, FLAG_NTEXTURE_ALPHA_IS_EFFECTIVE))
			return NTRUE;
		else
			return NFALSE;
	}
	...
	inline void NSetTextureGLFilters(NTEXTURE *ptexture, GLint minfilter_param, GLint magfilter_param)
	{
		ptexture->MinFilter = minfilter_param;
		ptexture->MagFilter = magfilter_param;
		glBindTexture(GL_TEXTURE_2D, ptexture->GL_Texture);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, minfilter_param);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, magfilter_param);
	}
	inline void NSetTextureGLMinFilter(NTEXTURE *ptexture, GLint minfilter_param)
	{
		ptexture->MinFilter = minfilter_param;
		glBindTexture(GL_TEXTURE_2D, ptexture->GL_Texture);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, minfilter_param);
	}
	inline void NSetTextureGLMagFilter(NTEXTURE *ptexture, GLint magfilter_param)
	{
		ptexture->MagFilter = magfilter_param;
		glBindTexture(GL_TEXTURE_2D, ptexture->GL_Texture);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, magfilter_param);
	...
	inline void NSetTextureGLWrap(NTEXTURE *ptexture, GLint wraps_param, GLint wrapt_param)
	{
		ptexture->WrapS = wraps_param;
		ptexture->WrapT = wrapt_param;
		glBindTexture(GL_TEXTURE_2D, ptexture->GL_Texture);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wraps_param);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrapt_param);
	}
	inline void NSetTextureGLWrapS(NTEXTURE *ptexture, GLint wraps_param)
	{
		ptexture->WrapS = wraps_param;
		glBindTexture(GL_TEXTURE_2D, ptexture->GL_Texture);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wraps_param);
	}
	inline void NSetTextureGLWrapT(NTEXTURE *ptexture, GLint wrapt_param)
	{
		ptexture->WrapT = wrapt_param;
		glBindTexture(GL_TEXTURE_2D, ptexture->GL_Texture);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, wrapt_param);
	...
	inline void NRegisterTextureReference(NTEXTURE *ptexture)
	{
		NErrorIf(ptexture->ReferenceCount == NU16_MAX, NERROR_VALUE_OUTOFRANGE);
		ptexture->ReferenceCount++;
	}
	inline void NUnregisterTextureReference(NTEXTURE *ptexture)
	{
		NErrorIf(ptexture->ReferenceCount == 0, NERROR_VALUE_OUTOFRANGE);
		ptexture->ReferenceCount--;
	}
