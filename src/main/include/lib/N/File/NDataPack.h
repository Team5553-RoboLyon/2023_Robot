...
extern "C"
{
	...
	typedef Nu32 (*NDP_WRITE)(void *pelement, NDATAPACKER *pdatapacker, FILE *pfile); // pointeur sur une fonction 'C' pour ecrire 'un �l�ment du type de celui g�r� par le serializale referencant ce pointeur sur fonction
	typedef Nu32 (*NDP_SREAD)(void *pvoid, NDATAPACKER *pdatapacker, FILE *pfile);	  // pointeur sur une fonction 'C' pour lire 'un �l�ment du type de celui g�r� par le serializale referencant ce pointeur sur fonction

	struct NDATAPACKHEADER
	{
		Nu32 Key;			 // Unique KeyCode
		Nu32 ElementSize;	 // ... Taille en octet d'un objet ( doit bien sur etre egal � "m_pArrayOfObject->elementSize" si "m_pArrayOfObject" existe ). Erreur si = 0 !!!
		Nu32 ArrayOfPtrSize; // ... de l'array de pointeurs, ( = nombre de pointeur written/read ) ! m_size = nombre d'objets alou�s en une fois pr�sents dans 'm_pArrayOfObject' + nombre d'objet isol� allou� 1 par 1
...
#define FLAG_NDATAPACK_LOCKED BIT_0
	...
	struct NDATAPACK
	{
		...
		Nu32 Processed;				 // nombre de pointeurs s�rializ�s trait�s ( written or read )
		NARRAY ArrayOfPtr;			 // Array de tous les pointeurs serializ�s de 'Key' : index du NLSERIAL dans m_serializableArray
									 // cet array contient TOUS les pointeurs de type type_id y compris ceux des objets pr�sents dans m_array !
