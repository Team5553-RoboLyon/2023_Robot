...
#define FLAG_NSKIN_DESC_BUILD_COLORS BIT_2
	...
	inline void NFreezeAllJoints(NSTRUCTURE *ps)
	{
		Nu32 i;
		NJOINT *pj;
		...
		{
			FLAG_ON(pj->Flags, FLAG_JOINT_FIXED);
		}
	}
	...
	inline void NUnfreezeAllJoints(NSTRUCTURE *ps)
	{
		Nu32 i;
		NJOINT *pj;
		pj = (NJOINT *)NGetFirstArrayPtr(&ps->JointArray);
	...
	inline void NSetJointPosition(NJOINT *pj, const NVEC3 *v)
	{
		pj->CurrentPosition = *v;
		pj->OldPosition = *v;
	}
	inline void NSetJointPositionf(NJOINT *pj, const Nf32 x, const Nf32 y, const Nf32 z)
	{
		NVec3Set(&pj->CurrentPosition, x, y, z);
	...
	inline void NResetJointDynamic(NJOINT *pj)
	{
		pj->OldPosition = pj->CurrentPosition;
	...
	*/
	...
	inline void NUpdateStructureSkins(NSTRUCTURE *ps)
	{
		NSKIN *psk = (NSKIN *)NGetFirstArrayPtr(&ps->SkinArray);
		for (Nu32 i = NGetArraySize(&ps->SkinArray); i != 0; i--, psk++)
		{
