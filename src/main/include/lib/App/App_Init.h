...
#define WM_MESSAGE_UPDATE_CURRENT_PATH_WORKBENCH_PTR	(WM_USER+3) //		APP_EXPLORER_WINDOW				pCurrentWorkbench					Send to all APP WINDOW(s) when current edited Path Workbench Ptr is updated ( les datas sont les mêmes, juste le pointeur est different ).
...
#define WM_MESSAGE_SET_CURRENT_SELECTED_TRJKEY			(WM_USER+7)		//	APP_EXPLORER_WINDOW				keyIndex							Send to all APP WINDOW(s) when current SelectedKey is updated ( = NVOID si pas de key selectionnée ).
...
//#define WM_MESSAGE_DELETE_CURRENT_ROBOT_SPECS			(WM_USER+51) //		APP_EXPLORER_WINDOW				pDriveTrainSpecifications			Send to all App WINDOW(s) just after creation.		! Pas encore utilisé car une seule DTSpec pour l'instant !
//#define WM_MESSAGE_UPDATE_CURRENT_ROBOT_SPECS			(WM_USER+52) //		APP_EXPLORER_WINDOW				&DriveTrainSpecifications			Send to all App WINDOW(s) just after creation.		! Pas encore utilisé car une seule DTSpec pour l'instant !
...
#define WM_MESSAGE_KEY_INSERTED_AND_SELECTED			(WM_USER+200)//		NLPATH_WORKBENCH*				index in Trajectory->m_keysArray	Send to Window Explorer by a Tool. Une nouvelle clef a été insérée et selectionnée. L'index passé est celui de la clef insérée.
#define WM_MESSAGE_KEY_SELECTED							(WM_USER+201)//		NLPATH_WORKBENCH*				index in Trajectory->m_keysArray	Send to Window Explorer by a Tool. Une nouvelle clef a été sélectionnée (cliquée). L'index passé est celui de la clef.
#define WM_MESSAGE_KEY_DELETED							(WM_USER+202)//		NLPATH_WORKBENCH*				index in Trajectory->m_keysArray	Send to Window Explorer by a Tool. Une clef a été supprimée.  L'index passé est celui de la clef supprimée.
//#define WM_MESSAGE_KEY_MOVESTART						(WM_USER+203)//		NLPATH_WORKBENCH*				index in Trajectory->m_keysArray	Send to Window Explorer by a Tool. Une clef vient d'être cliquée avec l'outil MOVE/EDIT. Elle n'a pas encore été déplacée. L'index passé est celui de cette clef.
#define WM_MESSAGE_KEY_MOVED							(WM_USER+203)//		NLPATH_WORKBENCH*				index in Trajectory->m_keysArray	Send to Window Explorer by a Tool. Une clef a été déplacée.  L'index passé est celui de la clef déplacée.


//#define WM_MESSAGE_ACTION_ID							(WM_USER+203)//		NLPATH_WORKBENCH*				index in Trajectory->m_keysArray	Send to Window Explorer by a Tool. Une clef a été déplacée.  L'index passé est celui de la clef déplacée.

#define WM_MESSAGE_SET_CURRENT_ACTION_WINDOW			(WM_USER+1000)//																		Message local utilisé uniquement pour communiquer entre les fenetres "key Properties" et "Action"
#define WM_MESSAGE_GET_CURRENT_ACTION_WINDOW			(WM_USER+1001)//																		Message local utilisé uniquement pour communiquer entre les fenetres "key Properties" et "Action"

#define WM_MESSAGE_UPDATE_ACTION_WINDOW_CONTENT			(WM_USER+1002)//	TRUE or FALSE					WND_KEYACTION_CREATE_DATA*			Message local utilisé uniquement pour communiquer entre les fenetres "key Properties" et "Action"
#define WM_MESSAGE_DISPLAY_UPDATED_ACTION_TIMESHIFT		(WM_USER+1003)//	0								0									Message local utilisé uniquement pour communiquer entre les fenetres "key Properties" et "Action"

#define WM_MESSAGE_TRAJECTORY_REBUILDED					(WM_USER+1100)//	NLPATH_WORKBENCH*																	Message local utilisé uniquement pour communiquer entre les fenetres "key Properties" et "Action"
#define WM_MESSAGE_PATH_REBUILDED						(WM_USER+1101)//	NLPATH_WORKBENCH*																	Message local utilisé uniquement pour communiquer entre les fenetres "key Properties" et "Action"

#define WM_MESSAGE_VIRTUAL_ROBOT_UPDATED				(WM_USER+1102)//	NLPATH_WORKBENCH*																	Message local utilisé uniquement pour communiquer entre les fenetres "key Properties" et "Action"

typedef enum
{
	// prefixes des ID de fenêtres ( en place , uniquement pour plus de clarté )
	// APP_... pour application, concerne les fenêtres plus "globales".
	// WB_ ... pour WorkBench, concerne les fenêtres ayant attrait au WorkBench.
	// RB_ ... pour Robot, concerne les fenêtres ayant attrait aux params du robot.
...
// NL2DOCS_COORDS_STYLE_1.......................... Defini et utilisé pour les valeurs en ordinnées sur les NL2DOCS
