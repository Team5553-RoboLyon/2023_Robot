...
struct NLPID_ERROR
{
	...
	inline void update(const Nf32 error) { m_previous = m_current; m_current = error; m_derivate = m_current - m_previous; m_integral += error; }
	Nf32 m_current;		// erreur actuelle
	Nf32 m_previous;	// erreur précédente
