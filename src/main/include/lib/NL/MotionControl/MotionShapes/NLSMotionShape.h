...
struct NLSMOTIONSHAPE
{
	...
	Nf32 m_vx; // Quantit� de vitesse acquise quand l'acceleration (initialement nulle) atteind sa valeur maximum ( soit m_ax ).
	Nf32 m_tx; // Dur�e n�c�ssaire pour que l'acceleration (initialement nulle) atteigne sa valeur maximum ( soit m_ax ).
	Nf32 m_sx; // PORTION fixe  de la distance ind�pendante de v0 et v1 couverte pas la SShape sur une dur�e de m_tx

	Nf32 m_tb; // Dur�e de maintient de l'acceleration maximum.

	inline void null() { Nmem0(this, NLSMOTIONSHAPE); }

	STYPE forwardBuildUp(const NLVSTAGEX *pv0, const Nf32 v1);
	STYPE forwardBuildUp(const NLKINLIMITS *pkmax, const Nf32 s0, const Nf32 v0, const Nf32 v1);	// setup � utiliser si on sait que v0 < v1.		Up car on "monte" de v0 vers v1.
	STYPE backwardBuildDown(const NLKINLIMITS *pkmax, const Nf32 s1, const Nf32 v0, const Nf32 v1); // setup � utiliser si on sait que v0 > v1.		Down car on "descend" de v0 vers v1.
