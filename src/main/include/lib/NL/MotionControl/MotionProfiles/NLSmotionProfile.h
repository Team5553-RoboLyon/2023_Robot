 ...
 *	Ces valeurs sont une asssociation des BIT 0 � 3.
...
// NCOLORPICKPACK sera sollicit� par les fonctions de tracage pour fournir 6 couleurs ( 0 � 5 ), il convient donc d'utiliser le format 4Bits (.. ou 2Bits )
...
struct NLSMOTIONPROFILE
{
	...
	Nf32 m_dt; // Dur�e totale du motion profile 'BRUT'.					|__ ! C'est � dire sans prise en compte de CutIn et CutOut
	...
	{
		Nmem0(this, NLSMOTIONPROFILE);
		m_dt = NF32_MAX;
		ppromote = promoteNull;
#ifdef _NEDITOR
		pdraw = drawNull;
#endif
	};

	inline Nf32 getEffectiveTimeSpan() const
	{
		return m_dt + m_cutInOffsets.m_dt + m_cutOutOffsets.m_dt;
		NErrorIf(m_cutInOffsets.m_dt > 0.0, NERROR_SYSTEM_CHECK);
		NErrorIf(m_cutOutOffsets.m_dt > 0.0, NERROR_SYSTEM_CHECK);
	};																									  // = m_dt auquel on enl�ve les cutin et cutout ( les valeurs de cutin et cutout sont tjrs n�gatives)
	inline Nf32 getEffectiveLength() const { return m_ds + m_cutInOffsets.m_ds + m_cutOutOffsets.m_ds; }; // = m_ds auquel on enl�ve les cutin et cutout ( les valeurs de cutin et cutout sont tjrs n�gatives)
