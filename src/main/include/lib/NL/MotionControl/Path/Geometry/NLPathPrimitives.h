...
#define NLPATH_PRIMITIVE_ID_NULL		0				/**<  ID de primitive indéfinie. Une primitive ne devrait jamais avoir cet ID.*/
...
#define FLAG_NLPATH_CLOTHOID_NXTLINK			BIT_0	/**<Une primitive "clothoide" flaggée de la sorte est considérée comme formant une "paire" avec la primitive suivante,
														*	qui est alors obligatoirement une clothoide. ! Cela arrive quand un Arc de cercle de base est remplacé par
														...
														*	arcs de cercles successifs par deux paires de clothoides, la premiere clothoide de la deuxieme paire "suit" bien la deuxième
														*	clothoide de la première paire, sans pour autant former une paire avec elle.) 
														*/

#define FLAG_NLPATH_CLOTHOID_PRVLINK			BIT_1	// Une primitive "clothoide" flaggée de la sorte est considérée comme formant une "paire" avec la primitive précedente ...
														// ( voir plus haut pour l'explication )
#define FLAG_NLPATH_CLOTHOID_SECOND				BIT_2	// Une primitive "clothoide" flaggée de la sorte est considérée comme étant "la deuxième". Une clothoide peut-être la "deuxieme" sans former une paire avec la premiere.
														// ... si il y a un cercle inséré.

#define FLAG_NLPATH_PRIMITIVE_RAW				BIT_3	// Une primitive "Arc" flaggée de la sorte est considérée comme 'BRUT'(RAW), c'est à dire qu'il s'agit de l'arc de cercle 'build' non remplacé par une paire de clothoides. 
														// Cela signifie que le coefficient ICRA ou ICRC est à ZERO. 
														// Une primitive "Segment" flaggée de la sorte est considérée comme 'BRUT'(RAW), c'est à dire qu'il relie directement 2 waypoints sans aucune construction de section.
														// Il s'agit de la plus simple representation d'une section: le segment direct.
														// Cela signifie que le coefficient BCR de la section est à ZERO. ( du coup aucune paires de cercles n'est construite ... )

#define FLAG_NLPATH_ARC_INDIRECT				BIT_4	// Une primitive "Arc" flaggée de la sorte est parcourue dans le sens indirect. Par défaut,un Arc est parcouru dans le sens direct et ce flag à OFF.
														// Le sens de parcours de l'Arc découle bien évidement du sens de parcours du cercle de construction dont il est issu.
 ...
 * \brief	Coeur commun à toute les primitives.
 *
...
typedef struct
{
	...
	Nu8			m_flags;			
	Nu16		m_keyPoint0;				// Index du premier keypoint associé à cette primitive dand l'array des keypoints. Le deuxième keypoint associé ayant pour index [m_keyPoint0+1]
...
{
	NLPATH_PRIMITIVE_CORE	m_core;
	Nf32					m_phi;			// Angle de démarrage de l'arc de cercle
	Nf32					m_omega;		// Longueur angulaire de l'arc de cercle
	Nf32					m_radius;
	NVEC2f32				m_center;
	NVEC2f32				m_i;			// vecteur i unitaire formant avec "m_j" une base orthonormée utilisée pour définir les coordonnées des points de l'arc
	NVEC2f32				m_j;			// vecteur j unitaire formant avec "m_i" une base orthonormée utilisée pour définir les coordonnées des points de l'arc
}NLPATH_ARC;

typedef struct
{
	NLPATH_PRIMITIVE_CORE	m_core;
	Nf32					m_km;			// courbure de la clothoide à son extrémité ( fin, l'autre extrémité à toujours une courbure nulle )
	Nf32					m_sigma;		// paramètre de clothoide ... -- Ref. [2.4] page 61. [K(s) = m_sigma*s] ... la courbure k en fonction de l'abscisse curviligne s.
	Nf32					m_param;		// paramètre de clothoide ... -- Ref. [1.6] page 42 precalculé permettant avec "sm" de calculer les coordonnées de tous les points de la clothoide.
	NVEC2f32				m_i;			// vecteur i unitaire formant avec "m_j" une base orthonormée utilisée pour définir les coordonnées des points de la clothoide
	NVEC2f32				m_j;			// vecteur j unitaire formant avec "m_i" une base orthonormée utilisée pour définir les coordonnées des points de la clothoide
...
//		La courbure K est stockée dans les keypoints.
//		
//		pour une PRIMITIVE SEGMENT: k est constante et nulle ! Elle est stockée dans le keypoint 1 du segment.
//					PRIMITIVE SEGMENT->kp0.k	= courbure de la primitive précédente.
//					PRIMITIVE SEGMENT->kp1.k	= courbure de cette primitive segment, ( vaut 0 )
//
//		pour une PRIMITIVE ARC: k est constante ! Elle est stockée dans le keypoint 1 de l'Arc.
//					PRIMITIVE ARC->kp0.k	= courbure de la primitive précédente.
//					PRIMITIVE ARC->kp1.k	= courbure de cette primitive Arc
//
//		pour une PRIMITIVE CLOTHOIDE: k varie de k0 à k1 !
//					PRIMITIVE CLOTHOIDE->kp0.k		= courbure en début de clothoide ( = courbure de la primitive précédente ).
