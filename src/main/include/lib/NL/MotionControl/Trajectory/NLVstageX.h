...
#include "../../MotionControl/Path/Geometry/NLPathPoint.h"
...
typedef struct NLVSTAGE NLVSTAGE;		// forward declaration ( NLVStage.h sera déclaré dans NLVstage.cpp)
...
struct NLVSTAGEX
{
	...
	NLSSTITCH		m_stitch;			// les delta de temps d'abscisse et de vitesse à ajouter au vstage pour partir/arriver lors d'un overshoot
	
	Nf32			m_s0Prime;			// "distance à parcourir" à la vitesse plafonnée par le VStage.
	Nf32			m_s1Prime;			// Initialement cette distance vaut "m_s1 - m_s0" mais,
												//	En cas de plafonnement à gauche elle vaut "m_s1 - s_left_out"
												//	En cas de plafonnement à droite elle vaut 0
												//  En cas d'overshoot à droite ou à gauche elle vaut 0
	NLKIN			m_kinXIn;
	NLKIN			m_kinXOut;


	//Nf32			m_axIn;					// Acceleration (<0) conseillée en entrée de VStage ( depuis a = 0 ), calculée afin de ne pas "dépasser la limite de vitesse"
											// fixée par la pente V(s) de la clothoide entrante et "descendante".
	//Nf32			m_axOut;				// Acceleration conseillée en sortie de VStage ( depuis a = 0 ), calculée afin de ne pas "dépasser la limite de vitesse"
											// fixée par la pente V(s) de la clothoide sortante et "montante".
//	Nf32			m_jxIn;
//	Nf32			m_jxOut;
	...
	*/
	...
	inline		Nf32 getPrimeFlatShapeTimeSpan() { Nf32 s = m_s1Prime - m_s0Prime; if ((!s) || (!m_v))return 0.0f; else return s / m_v; };
	inline		void setPrimeFlatShapeS0(const Nf32 s0) { /*NErrorIf(m_s1Prime - s0 < 0.0f, NERROR_SYSTEM_GURU_MEDITATION);*/ m_s0Prime = s0;}
	inline		void setPrimeFlatShapeS1(const Nf32 s1) { /*NErrorIf(s1 - m_s0Prime < 0.0f, NERROR_SYSTEM_GURU_MEDITATION); */m_s1Prime = s1;}
