...
#define NLTRAJECTORY_CHUNK_GETTIME_UNIQUE		 0	//  l'intervalle contient 2 fois la même valeur. Elle est unique et valide. le chunk est de type T. et s est inclu dan sle chunk.
#define NLTRAJECTORY_CHUNK_GETTIME_INTERVAL		 1	//  l'intervalle est un VRAI intervalle avec des valeurs start et end distinctes et valides. Le chunk est de type R et S est égal à celui du chunk (unique ).
#define NLTRAJECTORY_CHUNK_GETTIME_BEFORE		-1	// -l'intervalle contient 2 fois la même valeur. s se situe 'avant' le chunk. La valeur est la plus petite associée au chunk.
#define NLTRAJECTORY_CHUNK_GETTIME_AFTER		-2	// -l'intervalle contient 2 fois la même valeur. s se situe 'après' le chunk. La valeur est la plus grande associée au chunk.
...
public:
	...
	//virtual NLRAMSETEOUTPUT* output(NLRAMSETEOUTPUT*  poutput, const NLPATH_GEOMETRY* ppathgeometry, const Nf32 t) = 0;		// Méthode virtuelle pure ...	
	virtual void	initializePersistentData(NLTRAJECTORY_PACK_GETPOINT_PERSISTENT_RESULT *ppersist) = 0;	// Méthode virtuelle pure ...	
	virtual Nu32	read(FILE *pfile) = 0;			// Méthode virtuelle pure ...
	virtual Nu32	write(FILE *pfile) = 0;			// Méthode virtuelle pure ...

#ifdef _NEDITOR	
	virtual void	draw(NL2DOCS* p2docs, const NCOLORPICKPACK pickpack) = 0;// Méthode virtuelle pure ...
#endif												
	...
	Nf32	m_t0;									// Date de début de chunk. 
	...
	Nf32	m_s0;									// Abscisse curviligne de début de chunk.
