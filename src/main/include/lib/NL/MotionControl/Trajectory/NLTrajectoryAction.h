...
/* ... En conséquence, utiliser les macros et defines décrits dans "NLTrajectoryPointDesc.h"
...
#define FLAG_NLTRJACTION_COMMAND											BIT_5	// Une Action dont ce flag est ON prend le contrôle de la DT. La durée de cette prise de contrôle peut-être nulle ( instantanée )
...
// C'est à dire qu'il peut avoir une signification différente  selon l'ID de l'action.
...
struct NLTRJACTION_CORE
{
	Nu16	m_id;
	Nu16	m_flags;
	Nf32	m_timeShift;		// décalage temporel de déclenchement par rapport à l'Action precedente.
	Nf32	m_timeStamp;
	//Nu32	m_xData;			// lien (potentiel) vers des Données externes 
};
...
//	Nu32	m_xData;				// |
	...
	Nf32				m_angle;			// Angle a parcourir en degrés
	Nf32				m_speedRatio;		// vitesse max des moteurs en % de la vitesse max du robot.
	NLSMOTIONPROFILE	m_sMotionProfile;	// Motion Profile associé à la rotation sur place
};
...
struct NLTRJACTION_BACK_AND_FORTH
{
	...
	Nf32	m_pause;				// Temps de pause avant de revenir à position "Spot"
	Nf32	m_speedRatio;			// vitesse max des moteurs en % de la vitesse max du robot à l'aller et au retour.
};
// Taille : 5 X Nu32 + 32 x 32 bits = 160 bits + 1024 bits = 1184 bis [ 148 bytes ]
