...
#define FLAG_NLTRJKEY_LOCKED					BIT_0		// la clef ne peut être supprimée
#define FLAG_NLTRJKEY_STOP						BIT_1		// la clef est un stop
#define FLAG_NLTRJKEY_LINKABLE					BIT_2		// la clef peut être reliée à un point de chemin
//#define FLAG_NLTRJKEY_SPIN						BIT_3		// la clef contient au moins une action de rotation sur place
#define FLAG_NLTRJKEY_AUTO_STOP_TIMESPAN		BIT_4		// la durée 

#define FLAG_NLTRJKEY_DISABLE_BROKEN_LINK	BIT_13		// la clef est désativée car le lien avec le point de chemin "m_linkedPointId" n'a pu être réalisé.
#define FLAG_NLTRJKEY_DISABLE_TOO_CLOSE		BIT_14		// la clef est désativée car trop proche d'une autre
#define FLAG_NLTRJKEY_DISABLE_OUTOFPATH		BIT_15		// la clef est désactivée car elle a une abscisse hors du chemin ! Ce flag est réévalué à chaque build de trajectoire.
...
#define IS_NLTRJKEY_EDITABLE(flags)			(FLAGS_TEST(flags,FLAG_NLTRJKEY_LOCKED|FLAG_NLTRJKEY_DISABLE_BROKEN_LINK|FLAG_NLTRJKEY_DISABLE_TOO_CLOSE|FLAG_NLTRJKEY_DISABLE_OUTOFPATH,0))
...
#ifdef _DEBUG
	FLAG_OFF(pk->m_flags, FLAG_NLTRJKEY_LOCKED); // pour passer outre le crash provoqué en débug ( voir code du destructeur )
#endif
	pk->clear();
}
