...
#define NLTRAJECTORY_COLORPICKPACK_drawKinsArray NSTCPLT_4b_MPCK(0, 0, 0, 5, 3, 1, 0, 0, 15) // equivalant � NSTCPLT_4B_PCK(6,5,4,0,0,15)
#define NLTRAJECTORY_COLORPICKPACK_drawKinsArray_dark NSTCPLT_4b_MPCK(0, 3, 0, 4, 3, 0, 0, 0, 15)
#define NLTRAJECTORY_COLORPICKPACK_drawKinsArray_light NSTCPLT_4b_MPCK(1, 1, 0, 5, 3, 1, 0, 0, 15)
...
#define NLTRAJECTORY_COLORPICKPACK_drawVStages NSTCPLT_4b_MPCK(0, 0, 0, 10, 8, 3, 0, 0, 13)
...
#define FLAG_NLTRJ_ENFORCE_VEL BIT_0		// Force l'utilisation de 'pklim->m_v' par la fonction 'build' et ce m�me si elle est hors limite des capacit�s du Robot
#define FLAG_NLTRJ_ENFORCE_ACC BIT_1		// Force l'utilisation de 'pklim->m_a' par la fonction 'build' et ce m�me si elle est hors limite des capacit�s du Robot
#define FLAG_NLTRJ_ENFORCE_JRK BIT_2		// Force l'utilisation de 'pklim->m_j' par la fonction 'build' et ce m�me si il est hors limite des capacit�s du Robot
#define FLAG_NLTRJ_SPECIFIC_KINLIMITS BIT_3 // Le membre m_specificKinLimits est valide et doit �tre pris en compte avec les flags 'FLAG_NLTRJ_ENFORCE_xxx'
	// #define FLAG_xxx					BIT_3
	//											.
	//											.
	//											.
	// #define FLAG_NLTRJ_xxx					BIT_7	-->	MAX ! Car ces flags sont stock�s dans le Nu8 "NLPATH_WORKBENCH.m_trajectoryBuildFlags"
	...
	*/
	...
	Nu32 insertUserVelocityStage(NLVSTAGE *pnewvs);											  // Insert un NLVSTAGE ( = velocity stage d�fini manuellement par l'utilisateur )
	NLTRJKEY *insertUserKey(const Nf32 s, const Nu32 insert_before, const Nu32 keyflags = 0); // Insert une Trajectory Key (NLTRJKEY) d�finie par l'utilisateur.
																							  // NLTRJKEY*	insertUserKeyStop(const Nf32 s, const Nu32 insert_before);							// Insert une Trajectory Key (NLTRJKEY) qui est un Stop, d�finie par l'utilisateur.
	NLTRJKEY *addUserKey(const Nf32 s, const Nu32 keyflags = 0);							  // Insert une Trajectory Key (NLTRJKEY) d�finie par l'utilisateur.
																							  //	NLTRJKEY*	addUserKeyStop(const Nf32 s);													// Insert une Trajectory Key (NLTRJKEY) qui est un Stop, d�finie par l'utilisateur.
																							  //	NLTRJKEY*	MoveUserKey(const Nf32 s);														// d�place une Trajectory Key (NLTRJKEY) � une nouvelle position s et potentiellement la change de place dans l'array des keys !

	inline void setKinematicsTweaks(const NLKINTWEAK *pktwk)
	{
		if (pktwk)
		{
			m_kinTweak = *pktwk;
		...
		{
			m_kinTweak.m_aHoldLocalLimitRatio = 0.0f;
			m_kinTweak.m_jHoldLocalLimitRatio = 0.0f;
		}
	}
	void setPath(NLPATH *ppath);
	inline void setSpecificKinematicLimits(NLKINLIMITS *pklim, Nu32 enforceflags = 0)
	{
		if (pklim)
		{
			FLAG_ON(m_flags, FLAG_NLTRJ_SPECIFIC_KINLIMITS | (enforceflags & MASK_NTRJ_ENFORCE));
			m_specificKinLimits = *pklim;
		}
		else
		{
			FLAG_OFF(m_flags, FLAG_NLTRJ_SPECIFIC_KINLIMITS);
		}
	}
...
#ifdef _NEDITOR
	NARRAY *getLimitedpKeysArray()
	{
		return &m_limitedpKeysArray;
	}

	void drawKnodes(NL2DOCS *p2docs, const NCOLORPICKPACK pickpack);		// NCOLORPICKPACK associ� : NLTRAJECTORY_COLORPICKPACK_drawKnodes
	void drawVStages(NL2DOCS *p2docs, const NCOLORPICKPACK pickpack);		// NCOLORPICKPACK associ� : NLTRAJECTORY_COLORPICKPACK_drawVStages
	void drawMotionProfile(NL2DOCS *p2docs, const NCOLORPICKPACK pickpack); // NCOLORPICKPACK associ� : NLTRAJECTORY_COLORPICKPACK_drawKinsArray

	void drawKinTArray(NL2DOCS *p2docs, const NCOLORPICKPACK pickpack); // NCOLORPICKPACK associ� : NLTRAJECTORY_COLORPICKPACK_drawKinsArray
	// void	drawKinRArray(NL2DOCS* p2docs, const NCOLORPICKPACK pickpack);					// NCOLORPICKPACK associ� : NLTRAJECTORY_COLORPICKPACK_drawKinsArray
	void drawReverseVelocityCheck(NL2DOCS *p2docs, const NCOLORPICKPACK pickpack); // NCOLORPICKPACK associ� : NLTRAJECTORY_COLORPICKPACK_drawReverseVelocityCheck
	// void	drawChunksArray(NL2DOCS* p2docs, const NCOLORPICKPACK pickpack) { m_pack.drawChunksArray(p2docs, pickpack); };	// NCOLORPICKPACK associ� : NLTRAJECTORY_COLORPICKPACK_drawKinsArray
	void drawMotionProfilePathKeyPointId(NL2DOCS *p2docs); // pas besoin de color pickpack car utilise le system d'affichage de coordonn�es ( avec style ) de NL2DOCS.
	...
	NLKINTWEAK m_kinTweak;			 // Ratios d'ajustement de l'acceleration et du jerk appliqu�s � l'entr�e et sortie de chaque palier de vitesse.
	NLKINLIMITS m_specificKinLimits; // limites kinematics associ�es � la trajectoire ! ATTENTION ! Les limites effectivements utilis�es sont diff�rentes ...
									 ...
									 // ... et en prenant en compte egalement la vitesse de croisiere max possible sur le chemin associ�.
...
private:
	...
	NLPATH *m_pPath;		   // Path associ� � la Trajectoire.
	NARRAY m_keysArray;		   // Array des 'Keys' d�finis par l'utilisateur le long du chemin.
	NARRAY m_userVStagesArray; // Array des Vstages ( NLVSTAGE) d�finis manuellement par l'utilisateur
	...
	NARRAY m_limitedpKeysArray;		  // Array des (pointeurs sur) 'Keys' d�finis par l'utilisateur le long du chemin, limit�es � l'intervalle couvert par le chemin.
	NARRAY m_pathVStagesArray;		  // Array des Vstages comput�s � partir du chemin.
	NARRAY m_limitedUserVStagesArray; // Array des Vstages ( NLVSTAGE) d�finis manuellement par l'utilisateur, limit�s � l'intervalle couvert par le chemin.
	NARRAY m_combinedVStagesArray;	  // Array de la combinaison des Vstages d�finis manuellement par l'utilisateur ( et limit�s) et des VStages.
	NARRAY m_vStageXSlicesArray;

	//	NARRAY					m_sMotionProfilesArray;		// Array des motionprofiles en external data. (utilis� pour le calcul des kins de SpotChunk )
