...
{
	// Appel du destructeur pour chaque élément de pdst
	if (destructor_callback)
	{
...
// ------------------------------------------------------------------------------------------
/**
 *	@brief	Check l'integrité d'une Array par rapport à un descriptif 'Arraybounds'
 *			
 *	@param	parr l'Array à checker
 *	@param	pb arraybounds de reference
 *	@param	bautosetup_zero_array booléen qui, si il est défini à NTRUE, autorise la fonction à faire un setup 'basic' de l'Array UNIQUEMENT dans le cas ou celui-ci est
 *			entièrement à ZERO ( tous les membres de NARRAY sont égal à ZERO ). A l'issue de ce setup l'Array aura toujours tous ces membres à ZERO SAUF
 *			ElementSize qui aura pris la valeur de BOUND.ElementSize !
 *
 *	@return		NARRAYCHK_OK			Parfait tout est à l'identique ! ( taille d'un element, capacité et taille )
 *				NARRAYCHK_ZEROS			Tous les membres de Array sont égal à ZERO et le paramètre entrant 'bautosetup_zero_array' étant à NFALSE
 *										la fonction signale juste cette configuration sans faire de setup automatique.
 *				NARRAYCHK_NULLPTR		?! le pointeur parr passé en paramètre est NULL !	
 *				NARRAYCHK_INCONSISTENCY	Incohérence ! ElementSize est NON NULL et différent
 *				NARRAYCHK_CORRUPTED		Corrompu !	ElementSize de Array est NULL mais ces membres Size et/ou capacité NON NULL !!
 *				NARRAYCHK_AUTOSETUP		l'Array était non définie avec tous ses membres à ZERO ( elementSize, Size et capacity  = 0 )
 *										Le paramètre entrant 'bautosetup_zero_array' étant à NTRUE la fonction a effectué automatiquement un Setup simple 
 *										pour matcher avec ArrayBounds en taille d'élément uniquement.
 *										Aucune allocation mémoire n'est effectuée par la fonction. Cela signifie que maintenant parr->ElementSize = pb->ElementSize.
 *										La capacité et La taille de array restent donc à 0.
 *				NARRAYCHK_SIZ_DIFF		Array et Bounds ont la meme taille d'élément la meme capacité mais la taille differe
 *				NARRAYCHK_CAP_DIFF		Array et Bounds ont la meme taille d'élément, la meme taille mais la capacité differe
 *				NARRAYCHK_SIZCAP_DIFF	Array et Bounds ont la meme taille d'élément mais ni la meme capacité ni la meme la taille
 */
 // ------------------------------------------------------------------------------------------
Ns32 NCheckArrayIntegrity(NARRAY* parr, const NARRAYBOUNDS* pb, const Nbool bautosetup_zero_array)
{
	NErrorIf(!pb, NERROR_NULL_POINTER); // Check possible uniquement avec une structure BOUNDS définie !!!
	if (!parr)
	...
	if (parr->ElementSize == 0)
	{
		if (parr->pFirst || parr->Size || parr->Capacity)
		{
			...
			return NARRAYCHK_CORRUPTED; // Signifie mémoire corrompue ( peut-être tout simplement non initialisée, ou alors ... Aïe )
		}
		else
		{
	...
	{
		// check integrity
		// En effet, l'array semble "settée" car elementSize est NON NULL
		// reste à vérifier que cela correspond aux valeurs entrantes ( seule la taille d'un element est determinante, taille et capacité peuvent varier )...
		if (parr->ElementSize != pb->ElementSize)
			return NARRAYCHK_INCONSISTENCY; // Signifie incohérence: la taille de l'élement de l'array est différente de celle décrite dan sle Bounds
		else
		{
