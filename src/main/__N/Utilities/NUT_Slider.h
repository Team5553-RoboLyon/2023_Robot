...
#define FLAG_NUT_SLIDER_FIXED_BASE_POSITION			BIT_5	///< To fix the slider base at a specific position. 
															...
															///< ... and set it to OFF if user pass à NULL pointer.
...
{ 
	...
	if (bactivate) { FLAG_ON(ps->Flags, FLAG_NUT_SLIDER_LOCKDOWN); } else { FLAG_OFF(ps->Flags, FLAG_NUT_SLIDER_LOCKDOWN); }
}
...
{
	ps->LockedDownRect.left = left; ps->LockedDownRect.right = right; ps->LockedDownRect.bottom = bottom; ps->LockedDownRect.top = top;
	if (bactivate) { FLAG_ON(ps->Flags, FLAG_NUT_SLIDER_LOCKDOWN); } else { FLAG_OFF(ps->Flags, FLAG_NUT_SLIDER_LOCKDOWN); }
}
...
{
	if (pv)
	{ FLAG_ON(pslider->Flags, FLAG_NUT_SLIDER_FIXED_BASE_POSITION); pslider->Base = *pv; }
	else
	{ FLAG_OFF(pslider->Flags, FLAG_NUT_SLIDER_FIXED_BASE_POSITION); pslider->Base.x = 0.0f; pslider->Base.y=0.0f;}
}
