...
inline Nbool NIsADInterstitielLoaded(){if(ISFLAG_ON(NEngineCoreFlags,FLAG_NENGINE_CORE_AD_INTERSTITIAL_LOADED)){return NTRUE;}else{return NFALSE;}}
void NRequestADInterstitiel();
void NShowADInterstitiel();
//void NHideADInterstitiel() doesn'exist !

inline Nbool NIsADRewardedLoaded(){if(ISFLAG_ON(NEngineCoreFlags,FLAG_NENGINE_CORE_AD_REWARDED_LOADED)){return NTRUE;}else{return NFALSE;}}
