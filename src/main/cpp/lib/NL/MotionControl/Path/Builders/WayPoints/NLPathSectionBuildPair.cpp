...
void NLPATH_SECTION_BPAIR::setup(const NLPATH_WAYPOINT *pwp0, const NLPATH_SECTION_BCIRCLE *pc0, const NLPATH_WAYPOINT *pwp1, const NLPATH_SECTION_BCIRCLE *pc1)
{
	...
	NVEC2 o1p1; // Vecteur partant du centre 'o1' du cercle 1 et pointant sur le waypoint 0 situ� sur le cercle 1.
	NVEC2 p1e;	// Vecteur partant du waypoint 1 situ� sur le cercle 1 et pointant sur le point d'entr�e 'e' �galement situ� sur le cercle 1.

	NVEC2 o0p0; // Vecteur partant du centre 'o0' du cercle 0 et pointant sur le waypoint 0 situ� sur le cercle 0.
	NVEC2 p0s;	// Vecteur partant du waypoint 0 situ� sur le cercle 0 et pointant sur le point de sortie 's' �galement situ� sur le cercle 0.

	NVEC2 o0s; // Vecteur partant du centre 'o0' du cercle 0 et pointant sur le point de sortie 's' situ� sur le cercle 0.
	...
	NVEC2 i, j; // vecteurs unitaires d'une base orthonorm�e pour construire les points d'entr�e et de sortie en cas de type S
	NVEC2 vp;	// vecteur perpendiculaire � o0o1 et formant une base directe OU indirecte avec o0o1 selon le sens du cercle consid�r� lors du calcul.
	...
	if (pc0->sgn == pc1->sgn)
	{
		...
		// CALCUL DES POINTS D'ENTREE ET DE SORTIE 's' et 'e' situ�s respectivement sur le cercle 0 et le cercle 1
		//
		...
		if (pc0->sgn == 1)
		{
			...
			// point de sortie situ� sur le cercle 0 ( point o� la tangente touche le cercle )
			s.x = pc0->o.x + o0s.x;
			s.y = pc0->o.y + o0s.y;

			// entr�e sur cercle 1
			e.x = pc1->o.x + o0s.x; // |_ Car dans le cas d'une liaison en C et parceque les deux cercles ont le m�me rayon, nous avons "o0s = o1e"
			e.y = pc1->o.y + o0s.y; // |
		}
		else
		// Le cercle 0 est de sens direct:
		{
			...
			// entr�e sur cercle 1
			e.x = pc1->o.x + o0s.x; // |_ Car dans le cas d'une liaison en C et parceque les deux cercles int le m�me rayon, nous avons "o0s = o1e"
			e.y = pc1->o.y + o0s.y; // |
		}

		// CALCUL DES OUVERTURES ANGULAIRES 'beta0' et 'beta1'
		//
		// OUVERTURE ANGULAIRE beta0: angle parcouru depuis p0 jusqu'au point de sortie 's' situ� sur le cercle 0.
		o0p0.x = pwp0->p.x - pc0->o.x;
		o0p0.y = pwp0->p.y - pc0->o.y;

		// ... Utilisation du produit scalaire de 'p0s' avec 'u0' ( partant de p0 et pointant dans la direction du robot  ) pour d�terminer si la sortie 's' est 'devant' ou 'derriere' le point p0.
		p0s.x = s.x - pwp0->p.x;
		p0s.y = s.y - pwp0->p.y;
		...
		// OUVERTURE ANGULAIRE beta1: angle parcouru depuis le point d'entr�e 'e' situ� sur le cercle 1 jusq'au point p1.
		o1p1.x = pwp1->p.x - pc1->o.x;
		o1p1.y = pwp1->p.y - pc1->o.y;

		// ... Utilisation du produit scalaire de 'p1e' avec 'u1' ( partant de p1 et pointant dans la direction du robot  ) pour d�terminer si l'entr�e 'e' est 'devant' ou 'derriere' le point p1.
		p1e.x = e.x - pwp1->p.x;
		p1e.y = e.y - pwp1->p.y;
	...
	else
	{
		...
		// D'apres Thales:
		//					R0 / R1 = [o0,N]/[o1,N] = [M0,N]/[M1,N] Avec R0 et R1 les rayon des cercles consid�r�s qui sont DIFFERENTS ! [ Dans Ref. Ils sont �gaux ... :( ]
		//
		...
		//
		// Rien de plus facile � comprendre. Ce r�sultat se base sur le fait que:
		//
		//					R = p0p1/4 ( regle arbitraire, mais logique, fix�e par le programme )
		//					o1n = o0n = o0o1/2 ( car les deux cercles ont m�me rayon R )
		//
		...
		//
		// Dans la version de l'algorythme	que nous proposons ici, nous laissons la possibilit� aux utilisateurs de modifier la valeurs des rayons R0 et R1 ...
		...
		//					0 < R0 < (p0p1/4)
		//
		// Les rayons R0 et R1 seront calcul�s comme suit:
		//
		//					R0 = R1 = (p0p1/4) * k	avec k defini comme suit:	0 < k <= 1
		//
		// en cons�quence, pour le calcul de alpha nous retiendrons:
		//
		...
		// CALCUL DES POINTS D'ENTREE ET DE SORTIE 's' et 'e' situ�s respectivement sur le cercle 0 et le cercle 1
		//
		// d�finition de la base (i,j) avec,
		//					i partant du centre o0 du cercle 0 et pointant vers le centre o1 du cercle 1
		//					j vecteur orthogonal � i et for�ant avec lui une base directe.
		i = o0o1;
		NVec2Normalize(&i);
		j.x = -i.y;
		j.y = i.x;
		...
		// ... Utilisation du produit scalaire de 'o0p0' avec 'vp' ( vecteur perpendiculaire � o0o1   ) pour d�terminer ...
		// ... de quel c�t� de 'o0o1' se trouve le point 'p0'
		...
		// ... Utilisation du produit scalaire de 'o0p0' avec 'vp' ( vecteur perpendiculaire � o0o1   ) pour d�terminer ...
		// ... de quel c�t� de 'o1o0' se trouve le point 'p0'
		vp.x = -o1o0.y * pc1->sgn; // |_ Selon le sens du cercle on choisira de construire une base directe ou indirecte ( ici le schema est invers� par rapport � celui utilis� pour l'�valuation de beta0 )
		vp.y = o1o0.x * pc1->sgn;  // |
	...
	// Calcul de 'se' le vecteur partant du point de sortie 's'du cercle 0 et pointant sur le point d'entr�e 'e' sur le cercle 1.
	// La longueur 'lse' du vecteur 'se' ( avant normalisation ) correspond � la longueur du segment [se] tangent aux deux cercles.
	se.x = e.x - s.x;
	se.y = e.y - s.y;
...
// ------------------------------------------------------------------------------------------
/**
 *	@brief	Dessine les angles ALPHA d'une paire de cercles. Pour chaque cercle, les 2 points de contact des tangentes sont r�partis sym�triquement de par et d'autre de l'axe reliant le centre des
 *			deux cercles. Si pour un cercle on appelle M0 et M1 les deux points de contact,O1 le centre de ce cercle, et O2 le centre du deuxi�me cercle, on a alors:
 *			ALPHA = Angle[M0,O1,O2] = -Angle[M1,O1,O2]
 *			\b ATTENTION! Cette fonction n'existe qu'en mode _NEDITOR. Dans tout autre mode ( avec _NEDITOR non d�fini ) la fonction est ignor�e � la compilation est donc inexistante.
...
/**
 *	@brief	Dessine
 *			\b ATTENTION! Cette fonction n'existe qu'en mode _NEDITOR. Dans tout autre mode ( avec _NEDITOR non d�fini ) la fonction est ignor�e � la compilation et donc inexistante.
 *	@param	pcol Couleur.
 *	@return	rien.
 */
// ------------------------------------------------------------------------------------------
void NLPATH_SECTION_BPAIR::drawBetaAngles(const NLPATH_WAYPOINT *pwp0, const NCOLOR *pcolbeta)
{
	...
	if (pcircle0)
	{
		// Pour le premier cercle nous avons besoin de pwp0 car pwp0 est le point de d�part.
		// L'angle Beta0 se mesure donc depuis l'axe [o0,pwp0].
		// Le point situ� sur le cercle "� la fin de l'angle" sera le point s de sortie de cercle ...
	...
	if (pcircle1)
	{
		// Pour le second cercle nous avons besoin de e car e est le point d'entr�e (de d�part) sur ce cercle.
		// L'angle Beta1 se mesure donc depuis l'axe [o1,e].
		// Le point situ� sur le cercle "� la fin de l'angle" sera le point pwp1 de sortie de ce second cercle ...
...
void NLPATH_SECTION_BPAIR::draw(const NLPATH_WAYPOINT *pwp0, const NLPATH_WAYPOINT *pwp1, const NCOLOR *pcol)
{
	...
	// Tra�age des deux Arcs de cercles de construction associ�s � la paire
	...
	//
	// L'arc de construction 0 commence en pwp0 ( le point de d�part) et se termine au point de sortie S (evidement situ� sur le cercle 0...)
	// Les coordonn�es normalis�es du vecteur o0pwp0, interpr�t�es comme le cosinus et le sinus d'un angle via la fonction 'atanf2' permettent de d�finir l'angle de d�part dans un cercle trigo orient� sur
	// le "rep�re �cran"
	if (pcircle0)
	{
		o0pwp0.x = pwp0->p.x - pcircle0->o.x;
		...
		// Les coordonn�es normalis�es du vecteur o0s, interpr�t�es comme le cosinus et le sinus d'un angle via la fonction 'atanf2' permettent de d�finir l'angle de sortie ...
	...
	//
	// L'arc de construction 1 commence au point 'e', d'entr�e sur le cercle 1 et se termine au point 'pwp1'
	// Les coordonn�es normalis�es du vecteur o1e, interpr�t�es comme le cosinus et le sinus d'un angle via la fonction 'atanf2' permettent de d�finir l'angle de d�part dans un cercle trigo orient� sur
	// le "rep�re �cran"
	if (pcircle1)
	{
		o1e.x = e.x - pcircle1->o.x;
		...
		// Les coordonn�es normalis�es du vecteur o1pwp1, interpr�t�es comme le cosinus et le sinus d'un angle via la fonction 'atanf2' permettent de d�finir l'angle de sortie ...
