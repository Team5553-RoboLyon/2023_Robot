...
Nu32 NLPATH_SECTION::computeAndExtract(NLPATH_WAYPOINT *pwp0, NLPATH_WAYPOINT *pwp1, NLPATH_HEURISTIC heuristic,NLPATH_GEOMETRY *pgeom, const Nf32 arc_length_threshold)
{
	...
	// TODO: Recherche préalable d'une section de type P ( Point ) on extrait alors directement une primitive de type POINT !!!
	/*
	...
	*/
	...
	// Recherche préalable d'une section de type A ( Aligned )
	...
	// Identification préalable d'un BuildCircleRatio NULL ( ou négatif)
	// Se fait après la recherche du TYPE A, car dans un cas ( TYPE A) la liaison est considérée comme "bonne" et dans l'autre ( ratio rayon <= 0 ) comme Mauvaise
	if (pwp0->rr <= 0.0f)
	{
		// TODO: En cas de BuildCircleRatio NULL alors il faut ici insérer une(2?) primitive(s) de type POINT !!!!!
	...
	// Nous allons donc construire les 4 cercles et les 4 paires de cercles nécéssaires à la détermination ultérieure du type ( C ou S. )
	...
	// -------------------------------------------
	// Le vecteur ortho0 est le resultat du produit vectoriel du vecteur "Z",perpendiculaire au plan de l'écran et pointant "vers nous",  avec le vecteur 'pfrom->u'.
	...
	ortho0.x = -pwp0->u.y;	ortho1.x = -pwp1->u.y;
	ortho0.y =  pwp0->u.x;	ortho1.y =  pwp1->u.x;
	...
	// Vérification et Ajustement dans le cas de longueurs d'arc à parcourir inférieures au seuil fixé par "arc_length_threshold"
	if (arc_length_threshold)
	{
		if (NABS(m_pBestPair->beta0) * m_pBestPair->pcircle0->r < arc_length_threshold)
		{
			// on ajuste LA SORTIE DU CERCLE 0 pour qu'elle coincide avec pwp0 et, en conséquence, pwp1 se retrouve déplacé!
			//
			/*
			...
			NVEC2					s;			---> CHANGE : DEVIENT egal à pwp0->p !
			NVEC2					e;			---> CHANGE : SE PLACE 'AU BOUT' de se*lse
			NVEC2					se;			---> CHANGE : DEVIENT egal à pwp0->u !
			...
			*/
			...
			// mise à jour de la paire avec un beta0 = 0
			...
			// repositionnement de pwp1 pour conserver la cohérence avbec le cercle 1
			i = m_pBestPair->se;
			j.x = -i.y;
			j.y = i.x;
		...
		if (NABS(m_pBestPair->beta1) * m_pBestPair->pcircle1->r < arc_length_threshold)
		{
			...
			// mise à jour de la paire avec un beta1 = 0
	...
	if ( (m_pBestPair->beta0 >= NLPATH_SECTION_BETA_THRESHOLD) && (barc.m_Icr != 0.0f) )
	{
		...
		// beta0 represente la "distance angulaire" séparant p0 de s en tournant dans le sens du cercle0.
	...
	return adjusted; // 0,1 ou 2 selon le nombre d'ajustement(s) effectué(s) [ 1 ajustement = suppression d'un arc de cercle, jugé trop court ]
}


void NLPATH_SECTION::drawBuildingCircles(const NLPATH_WAYPOINT *pwp0, const NLPATH_WAYPOINT *pwp1, const NCOLOR * pcol)
{
	// Traçage des 4 cercles de construction utilisé par la section
	NVEC3f32			c,d;
	c.z = 0.0f;
	d.z = 0.0f;
	...
	// Tracage des 2 Way points associés à la section et des vecteurs "u" associés.
	c.x = pwp0->p.x;
	c.y = pwp0->p.y;
...
// ------------------------------------------------------------------------------------------
/**
 *	@brief	Dessine 0 à 4 des 4 building paires ( ... de cercles de construction reliés entre eux par une tangente ) contenus dans la section.
 *			\b ATTENTION! Il est possible qu'une paire ne soit pas valide, simplement car elle n'aura pas été définie, par exemple quand la section reliant 2 waypoints
 *			a choisi le segment ligne droite plutôt que la combinaison "Arc / segment / Arc". Dans ce cas, cette pair ne sera pas dessinée même si "what" le précise.
 ...
 *	@param	pwp1 Second way point utilisé lors de la construction de la building pair.
 *	@param	pcol Couleur utilisée pour dessiner.
 *	@param	what Les valeurs ( 0 ou 1 ) des Bits 0,1,2,3 spécifient quelles building pairs doivent être dessinées. ( BIT_0 à 1 = dessine la pair 0, BIT_1 à 1 = dessine la pair 1, ... )  
