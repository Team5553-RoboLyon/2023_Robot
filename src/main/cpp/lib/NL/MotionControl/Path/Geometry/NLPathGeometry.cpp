...
NLPATH_GEOMETRY& NLPATH_GEOMETRY::operator=(const NLPATH_GEOMETRY& from)
{
	...
	NErrorIf(NLclearNLPathPointInArrayCallBack != NULL, NERROR_SYSTEM_CHECK);		// Cela signifie que les path points possèdent maintenant une call back 'clear en array', donc qu'il y a de la mémoire à désallouer !!! 
	NErrorIf(NLclearNLPathPrimitiveInArrayCallBack != NULL, NERROR_SYSTEM_CHECK);	// Cela signifie que les path primitives possèdent maintenant une call back 'clear en array', donc qu'il y a de la mémoire à désallouer !!! 
...
//	header.m_flags					= m_flags;
	...
	// 3) écriture m_pathPointsArray à la main et en une fois
	...
	// 4) écriture m_primitivesArray à la main et en une fois
...
{
	NErrorIf(s < 0.0f, NERROR_VALUE_OUTOFRANGE); // inférieure à l'abscisse minimale
	NErrorIf(s > m_ds, NERROR_VALUE_OUTOFRANGE); // supérieure à l'abscisse maximale
	...
	// sera plus grande que la longueur totale du chemin !!! Cela est du à l'imprécision potentielle du calcul des KINs, car pour eux, le s est le resultat
	// de l'integration de la vitesse, de l'acceleration et du jerk au cours du temps ...
	// Pour l'instant ce cas n'est pas géré !!! IL VA FALLOIR LE FAIRE !!! SINON ...
	// .... plantage assuré avec la fonction NLPATH_GEOMETRY::getPathPoint, en effet la boucle "while (pkp1->s < s) { pkp1++; pprim++; }" peut engendrer un dépassement ! 
	// .... en debug, le test en entrée de fonction: "NErrorIf(s > m_ds, NERROR_VALUE_OUTOFRANGE)" crash si s est plus grand que l'abscisse max du path.
	// .... en release, la boucle "while (pkp1->s < s) { pkp1++; pprim++; }" générera un pkp1 hors de l'array ce qui est MAL MAL MAL !
	//
	//  Le gérer signifie rallonger un poil le chemin en rajoutant une primitive segment et un path point pour garantir que l'abscisse du dernier KIN du motionProfile
	//  soit inférieure ou égale à celle du chemin.
	//  Pourquoi le dernier Kin du MP ? Parceque le param "S" passé lors de l'appel à getPathPoint doit toujours générer un résultat valide quand ce S provient d'un des KINs calculés
	// lors de la creation du motion profile de déplacement le long de NLPATH_GEOMETRY
	// Recherche de S
...
{
	// NErrorIf(s > pkp1->s, NERROR_VALUE_OUTOFRANGE); // supérieure à l'abscisse de pkp1
...
//	NErrorIf(s < pkp0->s, NERROR_VALUE_OUTOFRANGE); // inférieure à l'abscisse de pkp0
	...
	case NLPATH_PRIMITIVE_ID_CLOTHOID:
		...
		{
			// une seconde clothoide est toujours "inversée" par rapport au sens du chemin ( elle commence avec une courbure non nulle et termine avec une courbure nulle )
...
void NLPATH_GEOMETRY::draw(const NMATRIX* pmx, const Nf32 path_width)
{
	...
	if (m_primitivesArray.Size)
	{
		...
		for (i = 0; i < m_pathPointsArray.Size; i++, pkp0++)
		{
			...
			v1.z =		  u.z * m_KeyPointExtend.x*2.0f; // est supposé être null
...
{
	NErrorIf(s0 < 0.0f, NERROR_VALUE_OUTOFRANGE); // inférieure à l'abscisse minimale
	NErrorIf(s0 > m_ds, NERROR_VALUE_OUTOFRANGE); // supérieure à l'abscisse maximale
	NErrorIf(s1 < 0.0f, NERROR_VALUE_OUTOFRANGE); // inférieure à l'abscisse minimale
	NErrorIf(s1 > m_ds, NERROR_VALUE_OUTOFRANGE); // supérieure à l'abscisse maximale
	...
	// Basic position / direction transformées dans la base 'pmx'	
	NVEC3f32			v0, v1;
	NVEC3f32			va, vb;
	NVEC3f32			u;
	v0.z = 0.0f;
	...
	*/
	...
	Nu32	jmx;								// nbre de steps pour le tracage des Arc et clothoides ( dépend des accuracy et de la longueur d'arc à tracer ... )
	...
	// Recul de 1 purement 'techniqie' pour 'coller' avec la boucle de traitement qui avance de 1 à chaque tour dès le début du premier tour.
	pprim--; pkp1--; 
	...
	while (sb < s1)
	{
		...
		// Définition des abscisses de l'intervalle de tracage 
		sa = sb;
		sb = NMIN(s1, pkp1->s);

		// tracage de l'intervalle [sa,sb] situé sur la primitive 'pprim'
		switch (pprim->m_core.m_id)
		{
		case NLPATH_PRIMITIVE_ID_SEGMENT:
			slocal_a = sa - pkp0->s;
		...
		case NLPATH_PRIMITIVE_ID_CLOTHOID:
			...
			if (ISFLAG_ON(pprim->m_core.m_flags, FLAG_NLPATH_CLOTHOID_SECOND))
			{
				...
				// nbre de segments:
				Nu32 jmx = (Nu32)(((Nf32)m_ClothoidDrawAccuracy * (sb - sa)) / pprim->m_core.m_l) + 2; // +2 à cause du test j < jmx, de tel sorte que j 'finisse' sur ...+1 !
			...
			else
			{
				...
				// nbre de segments:
				jmx = (Nu32)(((Nf32)m_ClothoidDrawAccuracy*(sb-sa)) / pprim->m_core.m_l) + 2; // +2 à cause du test j < jmx, de tel sorte que j 'finisse' sur ...+1 !
		...
		case NLPATH_PRIMITIVE_ID_ARC:
			...
			// nbre de segments:
			jmx = (Nu32)(((Nf32)m_ArcDrawAccuracy * ang_omega) / pprim->m_arc.m_omega) + 2; // +2 à cause du test j < jmx, de tel sorte que j 'finisse' sur ...+1 !
