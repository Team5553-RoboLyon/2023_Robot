...
Nu32 NLVstageArrayTools::insertVelocityStage(NARRAY* parray, NLVSTAGE* pnewvs, const Nu32 mode)
{
	...
	else
	{
		...
		for (i = 0; i < parray->Size; i++, pvstage++)
		{
			...
			//			1/ Ne pusher sur "tmp_out_array" que les parties � gauche ou � droite du cookiecutter
			//			2/ Forcer la d�coupe systematique que pnewvs aie une vitesse plus �lev�e ou plus basse ...
			...
			{
				// Tout ou partie de pvstage a �t� pouss� sur "tmp_out_array" en �tant � droite de pnewvs ...
				// tous les pvstage suivants ne pouvant �tre qu'encore plus � droite, on sort !
				_brk = 1;
				break;
			}
		}
		...
		if (_brk)
		{
			// On ins�re pnewvs juste avant le dernier vstage ins�r� ( et qui �tait donc � droite, soit apr�s, pnewvdesc )
			...
			if (i < (parray->Size - 1))
			{
				// Cela signifie qu'on est sorti de la boucle "for" via un break sans attendre la condition de sortie 'officielle' c�d : (i == parray->Size)
				// le i�me vstage de l'array 'parray' a bien �t� cutOut/ins�r� mais la boucle a �t� interrompue juste apr�s.
...
{
	NErrorIf(pin == pout, NERROR_INCONSISTENT_VALUES);	// pin et pout doivent �tre 2 arrays differents !
	...
	// Si l'intervalle [s0,s1] est entierement avant ou entierement apr�s l'intervalle [pv0->m_s0, pv1->m_s1] ...
	if (s1 <= pv0->m_s0 || (s0 >= pv1->m_s1))
		return 0;

	// L'intervalle [s0,s1] ne saurait �tre plus 'large' que l'intervalle [pv0->m_s0, pv1->m_s1] ...
	s0 = NMAX(s0, pv0->m_s0);
	s1 = NMIN(s1, pv1->m_s1);

	// A partir d'ici nous sommes surs que ...
	// L'intervalle [s0,s1] est inclus dans l'intervalle [pv0->m_s0, pv1->m_s1]
	// On cherche maintenant � encadrer plus finement [s0,s1]
	...
	// Pour Pv0 et Pv1, dans le cas #2 la fonction slice ... slice dans le vide ! ( c'est � dire entre 2 Vstages, soit une portion de clothoide reliant 1 segment et un arc de cercle.)
	...
	// ... et cr�er un vstage "point" pour chacun d'eux.
	NLVSTAGE	vstage;
	Nu32		first;
	Nu32		size = (pv1 - pv0) + 1;
	Nf32		v0, v1;
	...
	if (size)
	{
		...
		// Dans les cas autres que le cas 2, l'abscisse m_s0 de pv0 devient s0 ! Normal on a s0 appartient � [pv0->m_s0, pv0->m_s1] et on commence par s0 !
		if (!(s0 < pv0->m_s0))
		{
			pvout0->m_s0 = s0;
		}
	...
	else
	{
		...
		// Dans les cas autres que le cas 2, l'abscisse m_s1 de pv1 devient s1 ! Normal on a s1 appartient � [pv1->m_s0, pv1->m_s1] et on termine par s1 !
		pvout1->m_s1 = s1;
	}
 ...
 *			Pour se faire, les vstages de chaque array sont utilis�s comme cookiecutters pour d�couper ceux de l'autre array.
 *			Ainsi les Vstages de pin0 sont d�coup�s par ceux de pin1
 *			Ensuite les Vstages de pin1 sont d�coup�s par les VStages r�sultant de la d�coupe pr�c�dente.
 *			Pour finir l'ensemble de ces Vstages sont fusionn�s en 1 seule Array avec la certitude qu'il n'y a pas 
 *			juxtaposition de 2 NLVStages.
 *
 *	@param	pout est l'array o� sera stock� la combinaison des 2 autres.
 *	@param	pin0 est le premier array de NLVSTAGE
 *	@param	pin1 est le second array de NLVSTAGE
 *	@param	in0_cutters_desc d�crit comment construire les cutters des NLVSTAGE de pin0 quand ceux ci sont utilis�s comme cookiecutters.
 ...
 *	@param	in1_cutters_desc d�crit comment construire les cutters des NLVSTAGE de pin1 quand ceux ci sont utilis�s comme cookiecutters.
 *			Il y a deux valeurs possibles:	\b COMBINE_RESET_CUTTERS	les cutters seront simplement RESET ( et donc verticaux ).
 *											\b COMBINE_CHAINED_CUTTERS	les cutters seront construits par "chainage".

 *	@return	le nombre de NLVSTAGE cr��s et push�s sur pout:
 */
 // ------------------------------------------------------------------------------------------
...
{
	NErrorIf(pin0 == pout, NERROR_INCONSISTENT_VALUES);	// pin0 et pout doivent �tre 2 arrays differents !
	NErrorIf(pin1 == pout, NERROR_INCONSISTENT_VALUES);	// pin1 et pout doivent �tre 2 arrays differents !
	NErrorIf(pin0 == pin1, NERROR_INCONSISTENT_VALUES);	// pin0 et pin1 doivent �tre 2 arrays differents !
...
// 	|
	...
	if (!pin0->Size)
	{
		if (!pin1->Size)
		{
			return 0; // normalement pout est initialement 'vide' et aucun element n'a �t� ajout�.
		}
		else
		{
			// en debug nous sommes s�rs que pout->Size = 0 !
	...
	else
	{
		...
		{
			Le cas o� pin0->Size et pin1->Size sont tous les 2 non nulls.
			est simplement trait� juste en dessous, en dehors du test.
		}
		*/
	}
	...
	*/
	...
	//	a)	On commence par d�couper les NLVSTAGE de pin0 avec les NLVSTAGES de pin1 comme cookiecutters.
	//		Pour ne conserver que les portions effectives, c'est � dire les portions "non-masqu�es" des NLVSTAGE de pin0 par un NLVSTAGE de pin1.
	//		Les portions de NLVSTAGE masqu�es ( = cookies ) totalement ou en partie, sont retir�es.
	NARRAY	trimed_0_array;
	NSetupArray(&trimed_0_array, pin0->Size + 1, sizeof(NLVSTAGE)); // capacit� estim�e tr�s aproximative ...
	...
	for (i = 0; i < pin0->Size; i++, pvs++)
	{
		...
		for (j = 0; j < pin1->Size; j++, pcookiecutter++)
		{
			...
			if (trimed_0_array.Size)
			{
				...
				// La fonction cutOut est configur�e pour:
				//			1/ Ne pusher sur "trimed_0_array" que les parties � gauche ou � droite du cookiecutter
				//			2/ pusher le vstage tout entier sur "trimed_user_vs_array" en cas de coupe non initi�e et retourner alors "FLAG_CUTOUT_RIGHT"
	...
	//	b)	Ensuite, on fait l'inverse, c'est � dire qu'on va d�couper les NLVStages de pin1 pour ne conserver que les portions "non-masqu�es" par les NLStage de trimed_0_array
	//		( soit ceux de pin0 fraichement d�coup�s )
	NARRAY	trimed_1_array;
	NSetupArray(&trimed_1_array, pin1->Size + 1, sizeof(NLVSTAGE)); // capacit� estim�e tr�s aproximative ...
	...
	for (i = 0; i < pin1->Size; i++, pvs++)
	{
		...
		for (j = 0; j < trimed_0_array.Size; j++, pcookiecutter++)
		{
			...
			if (trimed_1_array.Size)
			{
				...
				// La fonction cutOut est configur�e pour:
				//			1/ Ne pusher sur "trimed_1_array" que les parties � gauche ou � droite du cookiecutter
				//			2/ pusher le vstage tout entier sur "trimed_user_vs_array" en cas de coupe non initi�e et retourner alors "FLAG_CUTOUT_RIGHT"
	...
	//		En prenant soin au passage de rassembler les trims contigus qui auraient la m�me vitesse, ce qui est tout � fait possible lors de cette fusion
	NLVSTAGE* ptrimed_0 = (NLVSTAGE*)trimed_0_array.pFirst;			// Pointeur VSTAGE "utilisateur" d�coup�
	NLVSTAGE* ptrimed_0_out = ptrimed_0 + trimed_0_array.Size;		// Valeur "hors tableau" ( = max + 1 )

	NLVSTAGE* ptrimed_1 = (NLVSTAGE*)trimed_1_array.pFirst;			// Pointeur VSTAGE "comput� (� partir du path)" d�coup�
	...
	while (1)
	{
		...
		if (ptrimed_1 == ptrimed_1_out)
		{
			...
			{
				// C'est qu'on a termin� !
				break;
			}
			else
			{
				// Il ne reste plus qu'� "empiler" les ptrimed_0 qui restent jusqu'au dernier !
		...
		{
			// Il ne reste plus qu'� "empiler" les ptrimed_1 qui restent jusqu'au dernier !
			ptrimed_1->aggregate(pout);
			ptrimed_1++;
			// ---> continue
		}
		// ptrimed_0 et ptrimed_1 sont valides tous les deux, il faut choisir lequel des deux ins�re ...
		else
		{
			// Les deux ont exactement la m�me abscisse d�part
			...
			if (ptrimed_1->m_s0 == ptrimed_0->m_s0)
			{
				...
				if (ptrimed_0->m_s1 == ptrimed_0->m_s0)
				{
					...
					if (ptrimed_1->m_s1 == ptrimed_1->m_s0)
					{
						...
						// Dans ce cas de figure in�dit et normalement impossible tant qu'on autorise pas les user vstage POINT.
						// Cependant, au cas ou cela �volue ... on gere le cas de mani�re simple et efficace:
					...
					{
						// On ins�re d'abord le point puis l'autre !
						ptrimed_0->aggregate(pout);
						ptrimed_1->aggregate(pout);
						ptrimed_0++;
						ptrimed_1++;
			...
			else if (ptrimed_0->m_s0 < ptrimed_1->m_s0)
			{
				...
				// Le Trim 0 est situ� AVANT le Trim 1
				ptrimed_0->aggregate(pout);
				ptrimed_0++;
				// ---> continue
			}
...
//	NErrorIf((NGetLastArrayPtr(pout)) && ((NLVSTAGE*)NGetLastArrayPtr(pout))->m_s1 > m_pPath->m_geometry.m_ds, NERROR_VALUE_OUTOFRANGE); // sup�rieure � l'abscisse maximale
//	|
//	|
//	+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
