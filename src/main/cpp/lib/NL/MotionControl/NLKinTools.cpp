...
void NLKinTools::AddBack(NARRAY *pkinarray0, const NARRAY *pkinarray1)
{
	...
	Nf32 s = pk0->m_s;
	Nf32 t = pk0->m_t;
	Nu32 first = pkinarray0->Size;									 // index du premier �l�ment � shift� une fois que celui-ci est ins�r� dans pkinarray0
	Nu32 last = first + pkinarray1->Size - 2;						 // -2 car on parle ici de l'index du dernier �l�ment ( on va en copier 'Size - 1').
	NArrayBufferPushBack(pkinarray0, pk1 + 1, pkinarray1->Size - 1); // -1 car on ne copie pas le premier �l�ment
