...
void *NGetDataPackerPointer(const NDATAPACKER *pdpacker, const Nu32 index, const Nu32 key)
{
	...
	else
	{
		...
		// Seul NLSERIALIZER::FLAGS::READ_ONLY doit �tre ON !!!!
		NErrorIf(1, NERROR_UNAUTHORIZED_ACTION);
...
Nu32 NReadDataPacker(NDATAPACKER *pdpacker, FILE *pfile)
{
	...
	// chaque loop correspond � "un tour complet" aupr�s de chaque serializable ou on va � chaque fois lire les donn�es � rattacher � un certain nombre de pointeurs de l'array de pointeurs
	// associ� au serializable. Ce nombre peut - �tre null , notament dans le cas de plusieurs Loops.
	// Exemple/explication:
	// Rappel:	Plusieurs loops, signifie que l'�criture d'un element d'un serializable 'n' � engendr� l'inscription d'un element � enregistrer dans un serializable 'n - k ' !!!
	// Il a donc fallu redemand� "un tour" pour enregistrer les donn�es associ�es � ce pointeur.
	// Lors de ce tour supl�mentaire, tous les serializables n'avaient pas forcement de nouveaux pointeurs r�f�renc� ! Ils n'avaient donc rien de plus � enregistrer.
	// A chaque "loop" :
	//						on passe aupr�s de chaque serializable
	//						on ecrit/lit  le nombre de pointeurs � traiter
	...
	for (Nu32 l = 0; l < loops; l++)
	{
		...
		for (i = 0; i < pdpacker->DataPackTableSize; i++, pdatapack++)
		{
			...
			// !ICI cela n'est pas comme � l'�criture !
			// Pas besoin de repartir � chaque fois de pFirst pour trouver le pointeur sur pointeur  suivant ...
			// ... il n'y aura pas de reallocations au sein de "m_arrayOfPtr" tout les pointeurs on d�j� �t� allou�s !!!
