...
void NMoveCameraPosition(const NVEC3 *v){NVec3AddTo(&NCamera.Pos,v);FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NMoveCameraPositionf(const Nf32 x,const Nf32 y,const Nf32 z){NCamera.Pos.x+=x;NCamera.Pos.y+=y;NCamera.Pos.z+=z;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NMoveCameraTargetPosition(const NVEC3 *v){NVec3AddTo(&NCamera.TargetPos,v);FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NMoveCameraTargetPositionf(const Nf32 x,const Nf32 y,const Nf32 z){NCamera.TargetPos.x+=x;NCamera.TargetPos.y+=y;NCamera.TargetPos.z+=z;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}

void NSetCameraPosition(const NVEC3 *v){NCamera.Pos=*v;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NSetCameraPositionf(const Nf32 x,const Nf32 y, const Nf32 z){NVec3Set(&NCamera.Pos,x,y,z);FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NSetCameraPositionXf(const Nf32 x){NCamera.Pos.x=x;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NSetCameraPositionYf(const Nf32 y){NCamera.Pos.y=y;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NSetCameraPositionZf(const Nf32 z){NCamera.Pos.z=z;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}

void NSetCameraTargetPosition(const NVEC3 *v){NCamera.TargetPos=*v;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NSetCameraTargetPositionf(const Nf32 x,const Nf32 y, const Nf32 z){NVec3Set(&NCamera.TargetPos,x,y,z);FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NSetCameraTargetPositionXf(const Nf32 x){NCamera.TargetPos.x=x;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NSetCameraTargetPositionYf(const Nf32 y){NCamera.TargetPos.y=y;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NSetCameraTargetPositionZf(const Nf32 z){NCamera.TargetPos.z=z;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}

void NCameraSetVectorUp(const NVEC3 *v){NCamera.Up=*v;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NCameraSetVectorUpf(const Nf32 x,const Nf32 y, const Nf32 z){NVec3Set(&NCamera.Up,x,y,z);FLAG_ON(NCamera.Flags,FLAG_NCAMERA_LOOKAT_HASTOBEUPDATED|FLAG_NCAMERA_AXIS_HASTOBEUPDATED);}
void NCameraSetFov(const Nf32 fov){NCamera.fFovY = fov;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_PERSPECTIVE_HASTOBEUPDATED);}
void NCameraSetAspectRatio(const Nf32 fratio){NCamera.fAspectRatio = fratio;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_PERSPECTIVE_HASTOBEUPDATED);}
void NCameraSetZNear(const Nf32 fnear){NCamera.fZNear = fnear;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_PERSPECTIVE_HASTOBEUPDATED);}
void NCameraSetZFar(const Nf32 ffar){NCamera.fZFar = ffar;FLAG_ON(NCamera.Flags,FLAG_NCAMERA_PERSPECTIVE_HASTOBEUPDATED);}
...
NMATRIX *NGetPerspectiveMatrix(NMATRIX *pmatrix)
{
	...
	pmatrix->f00 = (2.0f*NCamera.fZNear)/(xmax-xmin);	pmatrix->f10 = 0.0f;								pmatrix->f20 = (xmax+xmin)/(xmax-xmin);											pmatrix->f30 = 0.0f; 						
	pmatrix->f01 = 0.0f;								pmatrix->f11 = (2.0f*NCamera.fZNear)/(ymax-ymin);	pmatrix->f21 = (ymax+ymin)/(ymax-ymin);											pmatrix->f31 = 0.0f; 
	pmatrix->f02 = 0.0f;								pmatrix->f12 = 0.0f;								pmatrix->f22 = (NCamera.fZFar+NCamera.fZNear)/(NCamera.fZFar-NCamera.fZNear);	pmatrix->f32 = (2.0f*NCamera.fZFar*NCamera.fZNear)/(NCamera.fZFar-NCamera.fZNear);
	pmatrix->f03 = 0.0f;								pmatrix->f13 = 0.0f;								pmatrix->f23 = -1.0f;															pmatrix->f33 = 1.0f;
...
*/
	...
	NMATRIX mat0,mat1;
	mat0.f00 = NCamera.XAxis.x; mat0.f10 = NCamera.XAxis.y;	mat0.f20 = NCamera.XAxis.z;	mat0.f30 = 0.0f; 						
	mat0.f01 = NCamera.YAxis.x;	mat0.f11 = NCamera.YAxis.y;	mat0.f21 = NCamera.YAxis.z;	mat0.f31 = 0.0f; 
	mat0.f02 =-NCamera.ZAxis.x;	mat0.f12 =-NCamera.ZAxis.y;	mat0.f22 =-NCamera.ZAxis.z;	mat0.f32 = 0.0f;
	mat0.f03 = 0.0f;			mat0.f13 = 0.0f;			mat0.f23 = 0.0f;			mat0.f33 =  1.0f;
...
#ifdef _NIOS
	NViewPort.Width  = viewport[2]*2; !!! todo: check that it's WEIRD after all !!! take a look at CSS pixel size stackoverflow on the Web ...
		NViewPort.Height = viewport[3]*2; !!! todo: check that it's WEIRD after all !!! ....
#endif
