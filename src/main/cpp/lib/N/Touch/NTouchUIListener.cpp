...
void NUnbindTouchUIListener(NTOUCH_UI_LISTENER *puilistener)
{
	...
	NErrorIf(	ISFLAG_ON(puilistener->Listener.Flags,FLAG_NTOUCH_LISTENER_TO_BE_CLEAN)
		...
		||	puilistener->Listener.LTouchCount
		||	puilistener->Listener.PreviousLTouchCount 
		||	puilistener->Listener.LTouchCountIn
		||	puilistener->Listener.PreviousLTouchCountIn , NERROR_INCONSISTENT_VALUES);	// a listener to unbind can not have listened Touch Registered !!!!
