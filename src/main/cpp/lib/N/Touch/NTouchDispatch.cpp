...
inline void start_newline(Nu32 nb){printf("\n");for(Nu32 i=nb;i!=0;i--){printf("\t");}}
...
static inline void _gamestate_specific__NListenTouchMoveSequence(NTOUCH_LISTENER *plistener, NTOUCH_SEQUENCE *pstart_touchsequence, NTOUCH_SEQUENCE *pmove_touchsequence, NTOUCH_SEQUENCE *pcanceled_touchsequence, const NTOUCH_SEQUENCE *psource_touchsequence )
{
	...
	for(s=psource_touchsequence->TouchCount;s!=0;s--,ptindex++)
	{
		...
		for( l=touchbuffersize;l!=0;l--, plistened ++ )
		{
			...
			else if(	ISFLAG_ON(plistener->Flags,FLAG_NGAMESTATE_NTOUCH_LISTENER_CAPTURE_TOUCH_MOVE_RELEASED_BY_UI) 
						&& ISFLAG_OFF( _STREAMTOUCH(*ptindex).Flags,FLAG_NTOUCH_LOCK_GAMESTATE )
						&& plistener->LTouchBufferSize < plistener->LTouchCountMax				)
			{
		...
		if( NIS_NODE_VALID(pui->ChildrenList.pFirst,&pui->ChildrenList) )
		{
			...
			while( NIS_NODE_VALID(puichild,&pui->ChildrenList) )
			{
				if( NIsUIActive(puichild) 
					&& (!FLAGS_TEST(puichild->Flags,FLAG_NUI_LISTEN_TOUCH_EVENT_CAPTURE_START|FLAG_NUI_LISTEN_TOUCH_EVENT_CAPTURE_MOVE_IN, FLAG_NUI_LISTEN_TOUCH_EVENT_CAPTURE_MOVE_IN )) )
				{
			...
			while(stacksize)
			{
				if( (puichild = NUIClearWatchStackPop()) )
				{
					...
					recursive_count ++;
					start_newline(recursive_count);	printf("+");
					start_newline(recursive_count);	printf("|");
...
// SEND EVENT
		...
		else 
		{
			...
			if( NIS_NODE_VALID(puilistener->ChildrenList.pLast,&puilistener->ChildrenList) )
			{
				...
				{
					start_newline(recursive_count); printf("|");
					start_newline(recursive_count); printf("+-->%d/%d UI Listeners Pushed from UIListener ChildrenList to NUIClearWatchStack.", stacksize, NGetNodeListCount(&UIListener_List) );
					start_newline(recursive_count); printf("|");
					start_newline(recursive_count); printf("+-->START UI Pop and Recursive NTOUCH_CANCEL dispatching.");
				}
				else
				{

					start_newline(recursive_count);	printf("|");
					start_newline(recursive_count);	printf("+-->There is NO UI Listeners (UIListener ChildrenList is empty).");
					start_newline(recursive_count);	printf("	NO dispatching to UI Children.");
				}
				recursive_count ++;
				#endif
		...
		else 
		{
			...
			if( NIS_NODE_VALID(puilistener->ChildrenList.pLast,&puilistener->ChildrenList) )
			{
				...
				if(stacksize)
				{
					...
					start_newline(recursive_count); printf("+-->%d/%d UI Listeners Pushed from UIListener ChildrenList to NUIClearWatchStack.", stacksize, NGetNodeListCount(&UIListener_List) );
					start_newline(recursive_count); printf("|");
					start_newline(recursive_count); printf("+-->START UI Pop and Recursive NTOUCH_END dispatching.");
				}
				else
				{
	...
	if( nevent_move.Touch.TouchSequence.TouchCount )
	{
		...
		if( NIS_NODE_VALID(puilistener->ChildrenList.pLast,&puilistener->ChildrenList) )
		{
			...
			if(stacksize)
			{
				...
				if( NIS_UI_EVENT_RESULT_DISPATCH(uiresult) )
				{	
					start_newline(recursive_count); printf("+-->START UI Pop and Recursive NTOUCH_MOVE dispatching.");
				}
				else
				{
					start_newline(recursive_count);	printf("+-->START UI Pop and Recursive NTOUCH_CANCEL dispatching.");
			...
			if( NIS_UI_EVENT_RESULT_DISPATCH(uiresult) )
			{
				while(stacksize)
				{
					if( puichild = NUIClearWatchStackPop() )
					{
						...
						recursive_count ++;
						start_newline(recursive_count);	printf("|");
						start_newline(recursive_count);	printf("+-->\tUI#%d[0x%p] pops from Stack.",stacksize,puichild);
						#endif
			...
			else
			{
				...
				while(stacksize)
				{
					if( puichild = NUIClearWatchStackPop() )
					{
						#ifdef _NOUT_SYS_EVENT
						start_newline(recursive_count);	printf("|");
						start_newline(recursive_count);	printf("+-->UI#%d[0x%p][%s] pops from Stack.",stacksize,puichild,puichild->pName);
						recursive_count ++;
						#endif
		...
		if( ISFLAG_ON(pui->Flags, FLAG_NUI_HIERARCHY_CAPTURE_TOUCH_MOVE_IN) 
			&& nevent_move.Touch.TouchSequence.TouchCount 
			&& ISFLAG_OFF(NEngineCoreFlags,FLAG_NENGINE_CORE_BLOCK_EVENT_TO_UI ) )
		{
...
//	Nu32	child_dispatch_end_return = 0;
	...
	//
	if( FLAGS_TEST(NEngineCoreFlags, FLAG_NENGINE_CORE_HIERARCHY_CAPTURE_TOUCH_MOVE_IN|FLAG_NENGINE_CORE_BLOCK_EVENT_TO_UI,FLAG_NENGINE_CORE_HIERARCHY_CAPTURE_TOUCH_MOVE_IN )
		&& ts_in.TouchCount )
	{
		#ifdef _NOUT_SYS_EVENT
		printf("\n\t|");
		...
		{
			if( NIsUIEnable(*ptr)
				&& (ISFLAG_ON((*ptr)->Flags,FLAG_NUI_LISTEN_TOUCH_EVENT_CAPTURE_MOVE_IN)||FLAGS_TEST((*ptr)->Flags, FLAG_NUI_LISTEN_TOUCH_EVENT_CAPTURE_START|FLAG_NUI_LISTEN_TOUCH_EVENT_CAPTURE_MOVE_IN|FLAG_NUI_HIERARCHY_CAPTURE_TOUCH_MOVE_IN,FLAG_NUI_HIERARCHY_CAPTURE_TOUCH_MOVE_IN) ) )
			{	
				NUIClearWatchStackPush(*ptr);
				stacksize ++;
			}
