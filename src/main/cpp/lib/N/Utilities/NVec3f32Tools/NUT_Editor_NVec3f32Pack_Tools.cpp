	...
	switch(pevent->Type)
	{
		...
		case NTOUCH_MOVE:
			...
			if(pv3)
			{
				...
				{
					// MOVE
					// pdata->SetPosition_CallBack(ptr sur l'array o� sont stock�s les elements, index de l'�l�ment , pointeur sur le NVEC3 inclu dans l'�l�ment slectionn� , pointeur sur la position 3D du curseur );
...
NEDITOR_RESULT NUT_NVec3f32PackTool_Insert(const NEVENT *pevent, NEDITOR_SELECTION *pselect, const Nu32 tool_param)
{
	...
	case NTOUCH_MOVE:
		...
		if (pv3)
		{
			...
			{
					// MOVE
					// pdata->SetPosition_CallBack(ptr sur l'array o� sont stock� les elements, index de l'�l�ment s�lectionn�, pointeur sur le NVEC3 inclu dans l'�l�ment , pointeur sur la position 3D du curseur );
					if (pdata->SetPosition_CallBack)
