				...
				if (hitem != NULL)
				{
					...
					if (PtInRect(&rect, localpt))
					{
						...
						case IDPOPUPMENU_CREATE_NEW_WAYPOINTS_PATH_WB:
							...
							_redrawTreeview(htreeview);
							// ... Après creation du nouveau Workbench on reaafecte pCurrentWorkbench à partir de son index calculé juste avant la création.
							if (Project.m_pCurrentWorkbench)
							{
								pw = (NLPATH_WORKBENCH*)NGetArrayPtr(&Project.m_workbenchArray, current_wb_index);
								if (pw != Project.m_pCurrentWorkbench)
								{
									Project.m_pCurrentWorkbench = pw;
						...
						case IDPOPUPMENU_DELETE_PATH_WB:
							...
							// à partir de son index calculé juste avant la suppression et potentielement décalé de 1 suite à la suppression.
							if (Project.m_pCurrentWorkbench)
							{
...
*/
	...
	case WM_MESSAGE_VIRTUAL_ROBOT_UPDATED:
		SendMessage(AppWindowsTable[APP_WINDOW_ENUM::RB_WIN_SIMULATION], WM_MESSAGE_VIRTUAL_ROBOT_UPDATED, wParam, lParam);
		return TRUE;
	}
