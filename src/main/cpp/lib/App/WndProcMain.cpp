...
extern HWND					hwnd_MotionProfileLayout;			// [Defini dans WndProcLayout.cpp]
...
LRESULT CALLBACK WndProc_Main(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDM_MOTIONPROFILE_LAYOUT:
			if (hwnd_MotionProfileLayout == NULL)
			{
				// Creation boite de dialogue additionnelle
				hwnd_MotionProfileLayout = CreateDialog(GetWindowInstance(hWnd), MAKEINTRESOURCE(IDD_DIALOG_LAYOUT), hWnd, (DLGPROC)WndProc_MotionProfileLayout);
				ShowWindow(hwnd_MotionProfileLayout, SW_SHOW);
			}
			break;
