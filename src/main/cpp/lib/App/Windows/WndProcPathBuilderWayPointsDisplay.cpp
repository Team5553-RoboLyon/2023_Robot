...
BOOL CALLBACK WndProc_PathBuilder_WayPointsDisplay(HWND hwndDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_CHECK_DRAW_PATH_HANDLES:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_HANDLES);
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_HANDLES);
			}
			return TRUE;
		...
		case IDC_CHECK_DRAW_BUILDING_CIRCLES:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_CIRCLES);
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_CIRCLES);
			}
			return TRUE;
		...
		case IDC_CHECK_DRAW_BUILDING_PAIR_ALPHA_ANGLES:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_ALPHA_ANGLES);
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_ALPHA_ANGLES);
			}
			return TRUE;
		...
		case IDC_CHECK_DRAW_BUILDING_PAIR_BETA_ANGLES:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_BETA_ANGLES);
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_BETA_ANGLES);
			}
			return TRUE;
		...
		case IDC_CHECK_DRAW_BUILDING_BASES:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_BASES);
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_BASES);
			}
			return TRUE;
		...
		case IDC_CHECK_DRAW_BUILDING_PAIR_LINES:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_LINES);
				
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_LINES);
			}
			return TRUE;
		...
		case IDC_CHECK_DRAW_BUILDING_PAIR_0:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_0);
				// infos ON:
				SetWindowText(GetDlgItem(hwndDlg, IDC_PATH_BUILDING_DETAILS_INFOS), WNDPROCPATHBUILD_DRAW_BUILDING_PAIR );
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_0);
				// infos OFF
		...
		case IDC_CHECK_DRAW_BUILDING_PAIR_1:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_1);
				// infos ON:
				SetWindowText(GetDlgItem(hwndDlg, IDC_PATH_BUILDING_DETAILS_INFOS), WNDPROCPATHBUILD_DRAW_BUILDING_PAIR);
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_1);
				// infos OFF
				SetWindowText(GetDlgItem(hwndDlg, IDC_PATH_BUILDING_DETAILS_INFOS), DEFAULT_INFOS);
			}
			return TRUE;
		...
		case IDC_CHECK_DRAW_BUILDING_PAIR_2:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_2);
				// infos ON:
				SetWindowText(GetDlgItem(hwndDlg, IDC_PATH_BUILDING_DETAILS_INFOS), WNDPROCPATHBUILD_DRAW_BUILDING_PAIR);
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_2);
				// infos OFF
				SetWindowText(GetDlgItem(hwndDlg, IDC_PATH_BUILDING_DETAILS_INFOS), DEFAULT_INFOS);
			}
			return TRUE;
		...
		case IDC_CHECK_DRAW_BUILDING_PAIR_3:
			...
			{
				FLAG_ON(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_3);
				// infos ON:
				SetWindowText(GetDlgItem(hwndDlg, IDC_PATH_BUILDING_DETAILS_INFOS), WNDPROCPATHBUILD_DRAW_BUILDING_PAIR);
			}
			else
			{
				FLAG_OFF(pbuilder->m_flags, FLAG_NLPATH_BUILDER_WAYPOINTS_DRAW_BUILD_PAIR_3);
				// infos OFF
				SetWindowText(GetDlgItem(hwndDlg, IDC_PATH_BUILDING_DETAILS_INFOS), DEFAULT_INFOS);
			}
			return TRUE;
