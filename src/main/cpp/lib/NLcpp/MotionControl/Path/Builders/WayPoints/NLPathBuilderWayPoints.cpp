...
NLPATH_BUILDER_WAYPOINTS::NLPATH_BUILDER_WAYPOINTS()
{
	...
	NSetupArray(&m_sectionsArray, (DEFAULT_NLPATH_WAYPOINTS_ARRAYCAPACITY - 1), sizeof(NLPATH_SECTION)); // le nombre de section est �gal au nombre de waypoints moins 1 !

	m_heuristic = NLPATH_heuristic_shortest; // par d�faut la fonction heuristic selectionnera le chemin le plus court.
	m_arcLengthThreshold = DEFAULT_NLPATH_WAYPOINTS_ARC_LENGTH_THRESHOLD;
#ifdef _NEDITOR
	// Partie additionnelle si _NEDITOR est actif
	// Ses donn�es membres sont utilis�es pour dessiner le path et ses attributs � l'�cran.
...
void NLPATH_BUILDER_WAYPOINTS::build(NLPATH *ppath)
{
	...
	NVEC2f32 relp; // position relative d'un waypoint par rapport � l'origine du chemin exprim�e dans la base monde ( = p - o )
	...
	//	   Cette nouvelle base (directe) est construite � partir du premier Waypoint de telle sorte que son vecteur u soit le vecteur J
	...
	// ... et on exprime le premier waypoint dans la base form�e par ...lui-m�me  ( = origine ) et la matrice associ�e au path
	wp.icr_A = pwp0->icr_A;
	wp.icr_C = pwp0->icr_C;
	wp.rr = pwp0->rr;
	wp.p.x = 0.0f;
	...
	for (i = 1; i < m_wayPointsArray.Size; i++, pwp0++)
	{
		...
		wp.p.x = relp.x * ppath->m_matrix.XAxis.x + relp.y * ppath->m_matrix.XAxis.y; // �quivalant �:  NVec2DotProduct(&relp, (NVEC2f32*)&ppath->m_matrix.XAxis);
		wp.p.y = relp.x * ppath->m_matrix.YAxis.x + relp.y * ppath->m_matrix.YAxis.y; // �quivalant �:  NVec2DotProduct(&relp, (NVEC2f32*)&ppath->m_matrix.YAxis);

		wp.u.x = pwp0->u.x * ppath->m_matrix.XAxis.x + pwp0->u.y * ppath->m_matrix.XAxis.y; // �quivalant �:  NVec2DotProduct(&pwp0->u, (NVEC2f32*)&ppath->m_matrix.XAxis);
		wp.u.y = pwp0->u.x * ppath->m_matrix.YAxis.x + pwp0->u.y * ppath->m_matrix.YAxis.y; // �quivalant �:  NVec2DotProduct(&pwp0->u, (NVEC2f32*)&ppath->m_matrix.YAxis);
		NArrayPushBack(&used_waypoints_array, (NBYTE *)&wp);
	}

	// 	Erase et (re)Construction des sections � partir des Waypoints
	...
	// On precise que maintenant la geometry n'est pas (plus) d�synchronis�e par rapport aux way points de construction.
 ...
 *			Si aucune section n'est trouv�e la fonction retourne NVOID
 *			Le test correct � r�aliser sur la valeur de retour de fonction est : if ( returned value == NVOID ) ERROR
 */
// ------------------------------------------------------------------------------------------
Nu32 NLPATH_BUILDER_WAYPOINTS::getSectionIndex(const Nu32 primitive_index)
{
	...
	// En mode DEBUG on crash ! Mais il ne s'agit pas d'une erreur, juste que l'utilisateur a pass� en param�tre un index de primitive
	// trop grand et qui ne correspond pas � une section de ce builder...
	// Il peut s'agir d'une erreur de coh�rence de donn�es, c'est � dire que l'index de la primitive est celui d'une primitive appartenant � un chemin
	// diff�rent de celui qui est/serait construit � partir de l'array de sections de ce builder ...
 ...
 *
 *	@param	pprim est un pointeur sur la primitive dont on veut connaitre la section d'origine
 *  @return valeur 32 bits compos�e de l'index ( 16 bits) de la section dont est issue la primitive et la partie dans la quelle se trouve cette primitive ( 0,1 ou 2 ) cod�e egalement sur 16 bits
 *			[ valeur retourn�e = NMAKELONG( index section, partie ) ]
 *			Si aucune section n'est trouv�e la fonction retourne NVOID
 *			Le test correct � r�aliser sur la valeur de retour de fonction est : if ( NHIWORD( returned value ) <= 2 ) OK else ERROR
 */
// ------------------------------------------------------------------------------------------
Nu32 NLPATH_BUILDER_WAYPOINTS::getSectionIndexXtd(const Nu32 primitive_index)
{
	...
	for (Nu32 i = 0; i < m_sectionsArray.Size; i++, psec++)
	{
		...
		if (primitive_index < pcount)
			return NMAKELONG(i, 0); // index de section combin� avec n� de partie ( ici 0 )

		pcount += GET_NLPATH_SECTION_PRIMITIVES_NB_B(psec->m_uid);
		if (primitive_index < pcount)
			return NMAKELONG(i, 1); // index de section combin� avec n� de partie ( ici 1 )

		pcount += GET_NLPATH_SECTION_PRIMITIVES_NB_C(psec->m_uid);
		if (primitive_index < pcount)
			return NMAKELONG(i, 2); // index de section combin� avec n� de partie ( ici 2 )
	}
	// En mode DEBUG on crash ! Mais il ne s'agit pas d'une erreur, juste que l'utilisateur a pass� en param�tre un index de primitive
	// trop grand et qui ne correspond pas � une section de ce builder...
	// Il peut s'agir d'une erreur de coh�rence de donn�es, c'est � dire que l'index de la primitive est celui d'une primitive appartenant � un chemin
...
{
	NErrorIf(!NVec2IsUnit(&pwp->u), NERROR_UNAUTHORIZED_VALUE); // u  doit �tre un vecteur unitaire.
...
{
	NErrorIf(!NVec2IsUnit(pdirection), NERROR_UNAUTHORIZED_VALUE); // pdirection  doit �tre un vecteur unitaire.
...
Nu32 NLPATH_BUILDER_WAYPOINTS::write(FILE *pfile)
{
	...
	// 3) �criture Array of WayPoints � la main et en une fois
