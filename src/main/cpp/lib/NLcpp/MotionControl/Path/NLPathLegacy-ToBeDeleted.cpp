...
{
	Nu16	Flags;
	Nu16	type;	// Type de transition: en S ou en C ou en A ( Align� ).

	Nf32	alpha;  // Transition en C et S. Les "points de contact entre les cercles et les tangentes" sont situ�s � un angle alpha de la perpendicaulaire � Ui. (! alpha = 0 pour trans.C )
	...
	NVEC2	s;		// point de sortie situ� sur le cercle 0
	NVEC2	e;		// point d'entr�e situ� sur le cercle 1
	NVEC2	se;		// vecteur 'sortie-entr�e' ( dont la longueur avant normalisation est l� longueur du segment "tangente" se)
	Nf32	lse;	// longueur du vecteur 'sortie-entr�e' calcul� avant normalisation.

	NVEC2	f0;		// point d'intersection des tangentes utilis�es pour le calcul des clothoides sur le cercle 0 -- Ref. [2.14] page 59 ( point F ) 
	NVEC2	f1;		// point d'intersection des tangentes utilis�es pour le calcul des clothoides sur le cercle 1 -- Ref. [2.14] page 59 ( point F ) 
	
	Nf32	l0;		// distance [p0,f0]
	Nf32	l1;		// distance [p1,f1]
	
	Nf32	clothoid_alpha0; // angle form� par les deux tangentes � l'arc de cercle 0. Soit U0 et se
	Nf32	clothoid_alpha1; // angle form� par les deux tangentes � l'arc de cercle 1. Soit U1 et se

	Nf32	clothoid_sigma0; // D�riv�e de la courbure de la clothoide au cercle 0 ( la deuxi�me clothoide au cercle 0 est symetrique � la premi�re, sa d�riv�e de courbure est la m�me au signe pr�s).
	Nf32	clothoid_sigma1; // D�riv�e de la courbure de la clothoide au cercle 1 ( la deuxi�me clothoide au cercle 1 est symetrique � la premi�re, sa d�riv�e de courbure est la m�me au signe pr�s).

	Nf32	clothoid_k0;	// courbure  au point de raccordement de la clothoide au cercle 0(... avec la deuxi�me clothoide qui lui est sym�trique ).
	Nf32	clothoid_k1;	// courbure  au point de raccordement de la clothoide au cercle 1(... avec la deuxi�me clothoide qui lui est sym�trique ).

	// Avec cercle ins�r� entre les deux clothoides
	...
	Nf32	clothoid_r0;		// rayon de l'arc de cercle ins�r� entre les deux clothoides � sommets symetriques  cr��es pour remplacer l'arc de cercle 0
	Nf32	clothoid_r1;		// rayon de l'arc de cercle ins�r� entre les deux clothoides � sommets symetriques  cr��es pour remplacer l'arc de cercle 1

	NVEC2	clothoid_o0;		// centre de l'arc de cercle ins�r� entre les deux clothoides � sommets symetriques  cr��es pour remplacer l'arc de cercle 0
	NVEC2	clothoid_o1;		// centre de l'arc de cercle ins�r� entre les deux clothoides � sommets symetriques  cr��es pour remplacer l'arc de cercle 1

	Nf32	clothoid_sigma0i; // D�riv�e de la courbure de la clothoide au cercle 0 avec cercle ins�r� de rayon 'clothoid_r0'
	Nf32	clothoid_sigma1i; // D�riv�e de la courbure de la clothoide au cercle 1 avec cercle ins�r� de rayon 'clothoid_r1'
...
void NLPATH_CirclePair(NLPATH_BUILDCIRCLE_PAIR *presult, const NLPATH_BUILDCIRCLE *pc0, const NLPATH_BUILDCIRCLE *pc1)
{
	...
	//Nf32	lo1o1, lo1p1, lo1qq; la longueur du vecteur o1o0 est la m�me que celle du vecteur o0o1, les longueurs des vecteurs o1p1 et o1q1 valent toutes les deux 'r' ( rayon "des" cercles) 
	...
	if (pc0->s == pc1->s)
	{
		...
		// beta0: angle parcouru depuis p0 jusqu'au point de sortie situ� sur le cercle 0.
		...
		//beta1: angle parcouru depuis le point d'entr�e situ� sur le cercle 1 jusq'au point p1.
	...
	else
	{
		...
		// Calcul des coordonn�es du "point de sortie" sur le cercle 0 et du "point d'entr�e" sur le cercle 1.
		NVEC2 i, j;
		i = o0o1;
		NVec2Normalize(&i);
		j.x = -i.y;
...
//beta1: angle parcouru depuis le point d'entr�e situ� sur le cercle 1 jusq'au point p1.
	...
	presult->f0	= xres.I;
//	NErrorIf(xres.ParamCoordAB < 0.0f, NERROR_INCONSISTENT_VALUES); // F est cens� se trouv� "devant" u0, c'est � dire dans la direction point�e par u0.
	presult->l0		= NABS(xres.ParamCoordAB); // comme 'u0' est unitaire, 'xres.ParamCoordAB' represente ici la longueur alg�brique du vecteur [p0,f0].
	...
	presult->f1	= xres.I;
//	NErrorIf(xres.ParamCoordAB > 0.0f, NERROR_INCONSISTENT_VALUES); // F est cens� se trouv� "derriere" u1, c'est � dire dans la direction oppos�e � celle point�e par u1.
	presult->l1		= NABS(xres.ParamCoordAB); // comme 'u1' est unitaire, 'xres.ParamCoordAB' represente ici la longueur alg�brique du vecteur [p1,f1].
	...
	// |																					|
	// +------------------------------------------------------------------------------------+
	// Calcul du rayon du "cercle ins�r�"
...
void NLPATH_DrawCirclePair(NLPATH_BUILDCIRCLE_PAIR *ppair)
{
	...
	// check de la courbure par deux moyens de calcul... ( peut-�tre songer � passer en 64 bits ??? )
	...
	// Tracage des points d'entr�e et de sortie
	va.z = 0.0f;
	va.x = ppair->s.x;
	va.y = ppair->s.y;
	NUT_Draw_Cross(&va, &xtd, &col);
	...
	// L'arc 0 commence en p0 ( le point de d�part) ...
	...
	// L'arc 1 commence en e ( l'entr�e ) ...
	oes.x = ppair->e.x - ppair->pc1->o.x;
	oes.y = ppair->e.y - ppair->pc1->o.y;
	NVec2Normalize(&oes);
	// et se termine en p1 ( le point d'arriv�e )
