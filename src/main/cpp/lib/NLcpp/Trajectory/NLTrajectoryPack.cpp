...
NLTRJPOINT_DESC_CFG NLTRAJECTORY_PACK::buildTravelingChunk(const NLTRJPOINT_DESC_CFG cfg, const NARRAY* pkinsarray)
{
	...
	else
	{
		...
		{
			// Chaque nouveau kin inséré est censé contribuer à un déplacement du début à la fin du chemin, du coup, tant que le trjpointdesc est de type travelling,
			// ce qui est le cas si m_pPrimitive est non null, l'abscisse curviligne de chaque nouveau kin inséré ...
...
void NLTRAJECTORY_PACK::postActionMessage(const NLTRJACTION* paction)
{
	...
	{
#ifdef _DEBUG
		NLTRJ_POSTED_MESSAGE* ppm = (NLTRJ_POSTED_MESSAGE*)NGetLastArrayPtr(&m_postedMessagesArray);
		if (ppm)
			if (ppm->m_timeStamp > paction->m_core.m_timeStamp)
...
Ns32 NLTRAJECTORY_PACK::getTime(NINTERVALf32* ptimerange, const Nf32 s)
{
	if (s < 0.0f)
	{
		ptimerange->start = ptimerange->end = 0.0f;
		return NLTRAJECTORYPACK_GETTIME_BEFORE;
	}
	else if (s > m_ds)
	{
		ptimerange->start = ptimerange->end = m_dt;
