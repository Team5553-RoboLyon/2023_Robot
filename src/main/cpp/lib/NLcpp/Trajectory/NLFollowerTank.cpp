...
void NLFOLLOWER_TANK::estimate(const Nf32 left_encoder_rev_count, const Nf32 right_encoder_rev_count, const Nf32 gyro_angle)
{
	...
	m_leftMove		= l - m_leftDistance;
	m_rightMove		= r - m_rightDistance;
	
	m_leftDistance	= l;
	m_rightDistance	= r;

	// mise à jour de la position et de l'angle "estimés" du robot.
	Nf32 v = IS_NLTRJPOINT_DESC_SET_DTMODE_REVERSE(m_persistent.m_pTrjPointDsc->m_flags) ? NLODOMETRY_DRIVETRAIN_V_FROM_WHEELS(m_rightMove, m_leftMove) : NLODOMETRY_DRIVETRAIN_V_FROM_WHEELS(m_leftMove, m_rightMove);
